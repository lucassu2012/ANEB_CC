...
dependencies {
    ...
    // ---- MockWebServer（PO 2026-09-07 批准；REVIEW L1-F8 仅剩的那一项）----
    // **为什么非它不可，以及为什么只剩这一项需要它**：L1-F8 原判断是四条路径都要一个
    // 真对端，实测**三条已闭合且全部没用它**（把内联在「要起真实连接才跑得到」的判据
    // 抽成纯函数直测）。剩下的两件够不着，因为它们要观测的**不是一个值，而是一个行为**：
    //   ① `uploadWindow` 调用点的供给层——实测四个实参各造一枚突变，**四枚全部存活**；
    //      其中「终点戳该取响应头到达还是最后一次 write 返回」**只有真的等一次响应头才分得开**；
    //   ② 请求侧——「窗口到点停止写入」「windowUnderrun」「chunked」，
    //      **没有一个会读的对端，就观测不到我们在到点那一刻停了笔**。
    // ⚠ 版本不在这里写：它经 `version.ref = "okhttp"` 与 okhttp 同步（见版本目录里的理由）。
