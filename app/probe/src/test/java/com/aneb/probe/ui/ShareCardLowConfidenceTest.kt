 ...
 *
 * **⚠ 上面这句已由实测坐实，不再是推理（2026-09-04，突变 M4）**：实际删掉 `render()` 里那
 * 15 行调用块后重跑本类，`compileDebugKotlin` 确有重编（不是 `UP-TO-DATE`——那种绿是空的），
 * 结果 **tests=3 failures=0 errors=0 ⇒ SURVIVED**。写下此条时它只是「看代码推出来的」，
 * 而**推理出的突变结论与实测出的，在文本里长得一模一样**。
 *
 * ## ⚠ 订正一条会把人送错方向的理由（原注写的挡路物不是真的那个）
 *
 * 原注写「没有 Robolectric native runtime、逐像素比对跑不起来」。**栅格化确实不可用，但那
 * 回答的不是这里要问的问题**：要答的是「`render()` 还调不调它」，**记录绘制调用就够，根本
 * 不需要画出像素**。真正的挡路物是**没有接缝**——`render(model: Model): Bitmap` 在函数体内
 * 自己 `Bitmap.createBitmap` ＋ `Canvas(bmp)`，外部拿不到那个 `Canvas`，无从观察它画了什么。
 * ⇒ **照原注去下载 native 构件，会白花一趟，而且仍然关不上这个缺口。**
 *
 * 可行的低价路子（**未采纳，留给属主决定**）：把绘制体抽成 `internal fun drawTo(c: Canvas,
 * model: Model)`，`render` 只管建位图再委托；测试传一个覆写 `drawText` 的记录型 `Canvas`，
 * 断言横幅文案确被画出。**没有替属主做这个决定**：它是为可测性改生产代码，而本件的闭合
 * 路径板面已指定为真机截图。**在那之前该缺口仍由装机后的真机分享图截图闭合，未闭合前
 * 不得当它已闭合。**
 */
