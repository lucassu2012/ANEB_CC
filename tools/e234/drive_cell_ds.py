...
⚠ 本文件＝ drive_cell.py（com.larus.nova 版）的 DeepSeek 孪生（D-638 ①）：时序逐字复制，只换 PKG 与四坐标。
⚠ 使用前必读：**下面四个 TAP_ 坐标是 P40（1200×2640）上 DeepSeek 的实测值（2026-09-01）**，
**四个全部由动作验证、不是目视**：新对话经「点后与冷启动状态逐位相同」判定，发送经「抽屉里出现该轮对话」判定，输入框经 mInputShown 由 false→true 判定，
...
TAP_MENU = ("1123", "232")      # 右上「+」＝新对话（DeepSeek 一下即可）
TAP_NEW = ("1123", "232")       # 同上：连点两次实测幂等，用来原样保住 drive_cell.py 的两拍时序
TAP_INPUT = ("290", "2370")     # 输入框（无键盘态）
TAP_SEND = ("1073", "1615")     # ↑ 发送（键盘弹起态）
