...
def dump_row_counts(text):
    ...
    ⚠ **〔2026-09-01 订正 · D-650②〕本段初版的理由被真机否证，照录以免有人再写一遍**：
    我原本写「一次 dump 可能『127 原始行、0 可用帧』⇒『环满但全待定』与『图层死了』
    是两种病」。**真机上那个组合不出现**：图层死则 `rows` 也归 0，两数**同向**、
    是同一件事的两面，**不是独立证据**。
    ⇒ 教训本身比结论值钱：**照抄一句话时，连它断言的那个状态存不存在也要验**
    （我与大脑写 D-646 时都没验那一支的可达性，同罪）。

    两个数仍然都要留，但理由是**它们回答不同的问题**，不是「分辨两种病」：
    · `dump_survival` 答「**有没有**取到帧」——抓灾难性失效；
    · `dump_rows` 答「取到的**够不够深**」——抓 `min>0` 而各段深浅不一的情形，
      而那正是 survival 的**构造性盲区**：它只问有无帧行、不问几行，
      于是深浅不一时 survival **恒 100%**（D-650② 实证，卡上分辨点已改为 min）。
    """
    out, cur = [], None
