...
#   · valid  —— **未作废、非验证**；**是否进统计由 `kind` 决定，不由本字段决定**
#     （原文写「实格，进统计」——「进统计」那半是错的：六个 `e234_dryrun_*`
#      正是 `state=valid`，而它们自己的 JSON 里写着「不得入任何统计池」。
#      同一个仓里两处对同一个词给出相反含义。这是「实格」那条的第三次发作：
#      **不是标错了对象，是那个词的定义本身多说了一件它没测量的事**。）
