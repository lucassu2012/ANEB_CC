...
- **分段归因**（`n1_rtt_p50_ms`；主要贡献段）：核心骨干 4 格；最大单项 P1/cmcc/busy/s1_chat·核心骨干=27ms；各段**均未见单点异常**（判据：与共同取值不等（**非 3σ**：过半单元取值相同））——最大单项落在该段分布内，不宜单独归因于该单元（单元间齐不齐见「分段异常定位」段）。
