...
def analyze(records, target=None, min_samples=cc.DEFAULT_MIN_SAMPLES):
    ...
    Campaign repeats are RUNS per cell, and usable records per cell is the only
    repeat evidence this layer can honestly give. D-340..343 added a 「不同重复」
    column keyed on `run.repeat_index` — but the contract defines `repeat_index`
    only inside a scenario (取证模式第几遍, forensic pass count, 快测恒 0), no
    producer writes it at run level, and 0 of 4822 real records carry it there.
    The column therefore read 「0 个不同重复(+N 无编号)」 on every real cell —
    inert, and implying the operator forgot to number repeats. Removed (D-344);
    crash-and-retry inflation is not machine-checkable under the current
    contract, and the runbook now sends that question to the field tally sheet.
    """
    ...
    for key in planned:
        ...
        cells.append({"cell": dict(zip(CELL_DIMS, key)), "samples": n,
                      "status": status})
    # same key set as `cells` minus `status`: a reader comparing the two tables
    # should not find one of them answering a question the other does not (D-341)
