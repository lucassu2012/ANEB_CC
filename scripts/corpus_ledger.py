...
def render_md(corpus, skipped, real, synth, st, bk, dbs, obs=()):
    ...
    if bk["aqs_runs"]:
        lines.append(
            ...
            "**引用本行时不得把它当作数据质量证据**。\n"
            "  > ⚠ **本台账无法从语料判定低置信的成因**：wire 记录是平铺的"
            "（`t1_ttft_ms`／`t2_itl_p95_ms`…），**不带逐 KPI 的 `sample_count` 与"
            "`low_confidence`**，run 级只留一个布尔 ⇒ 拿着这 "
            f"{bk['aqs_runs']} 个真实 run 也查不出是哪个 KPI 把它钉成低置信的，"
            "须查 `AqsInputMapper`。（写出来是为了省下下一个人徒劳查一遍的时间。）\n")
