...
if ($isRed -or $isFinalGreen) {
    ...
    #
    # **这条能活到今天，是因为当时清单没有读者**：全仓没有任何守卫/测试回读它。
    # ⚠ **该句已于 2026-09-07 失效，就地订正不删**（原文是「清单没有读者」的现在时断言）：
    #   现在有两个读者 —— `test_docs_commands.py` 的 presence 门（查条目在不在），
    #   与 `test_manifest_hashes.py`（承 D-822，查**四份**清单每条哈希与盘上文件是否相符）。
    #   括号里那句仍真：`check_evidence.py` 对 manifest/sha256/badges 三词**实测仍零命中**
    #   —— **读者不在它那里**，这正是原句会失效而没人发现的原因。
