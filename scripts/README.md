...
| 3a | **先跑 `tools/e234/e2_precheck.py`（`--run-dir` 指到**格目录**），再决定这一格要不要跑 e2**。三态（**专用判词，刻意不与四态共用**）：`NOT_APPLICABLE`＝该 App 结构性不适用（加轮数不解）；`CANNOT_TELL`＝**装置没看见**（记录太碎／两通道矛盾／A 侧不足）；`WORTH_RUNNING`＝值得跑。**前两者绝不可互代**；⚠ 它答的是「要不要跑」，`e2_analyze` 的四态答的是「跑了没有／跑出什么」——**曾因共用 `NOT_EXECUTED` 一词，把一格已出判词的实测盖掉且两边不报错** | 把「采集排期的洞」读成「App 的思考静默」——两者在去重后的帧序列里**长得一模一样**，而 e2 会照样出一个数 |
| 4 | 逐格跑 `e2_analyze`（`--run-dir` 指到**格目录**） | — |
| 5 | 取 `rule_matched` 等量时**先做差**（会话内累计，末行=该格合计） | 5 轮读成 1 轮 |
| 6 | 遇 `roi_mean=0.0` **先查该格是否息屏**，别判「ROI 作废」 | 方向完全反 |
| 7 | 核**本窗实际跑了哪几个功能**（以该批 README 为准，形状会临场缩减）；未跑的记 NOT_EXECUTED **并写明原因**；缺 `pcapdroid/` ⇒ 该功能**上行结论不给** | 把「没测」读成「没诉求」；或用别的通道估一个不存在的量 |
| 8 | 引用代码级语义时写明「据源码 @提交，装机构建 <版本>/<日期>」 | 源码与二进制不同源时，读数看起来完全正常 |
| 9 | 措辞：只写「前后两点实测同制式，格内瞬时漂移不可排除」，**不写「全程 NR」**；两个 `ttft_*` 写明用的哪个口径 | 说满了被推翻 |

## 下一批开窗**前**要落的六件（首窗判读页 §7 的可执行摘要）

> **本节是指针，不是叙述**：每条的来龙去脉与订正史见
> `docs/DOUBAO_WAVE0_JUDGMENT_20260830.md` §7，此处只留「开窗前要做什么」。
> 上一节（一页速查）管 T+1 判读，本节管**开窗前**——两者读者不同、时机不同。

1. **排格按「功能 × 条件」，不按功能**。逐格 `drop_reasons` 实证：**失败侧随条件翻面**
   （同一功能在 WiFi 与蜂窝下的瓶颈不在同一侧）。只看一个条件就下「该功能可解」会错。
2. **每格开跑前跑 `tools/e234/e2_precheck.py`**（见上节第 3a 步）。它把「跑完才知道能不能跑」
   变成开跑前几秒的检查，**且可以拿已有的格先验，不必先烧一格**。
3. **`--framestats-period-s` 必须从 20 改到 1**（本轮新测出的硬约束，§7 原文没有这条）。
   依据：`dumpsys SurfaceFlinger --latency` 的环缓冲**恒深 127 帧**，满帧率下只能覆盖
   `127 × 16.667ms ≈ 2.12s`（**16.667ms 是本批 `sf_latency` 首行实测＝60Hz；该机 60/90 双模式，
   90Hz 下这个界只有 1.41s——别把 2.12s 当常数搬走**，D-600）；
   周期 20s ⇒ 首窗五格里**24–45% 的相邻 dump 完全不重叠**，
   即**期间的帧永远丢了**。而丢帧与思考静默在去重后的序列里**不可区分**。
   `e2_precheck` 会按每格实测直接印出建议值，照它改即可。
   **它同时修掉第二个症状——「一轮只拿到一个环」**（两者是同一个溢出机制）：
   周期 20s 时，相邻 dump 要么**完全相同**（这段全空闲）、要么**完全不相交**（渲染超过 127 帧、
   中间的永远丢了），几乎没有中间态 ⇒ **无论轮多长，每轮只留下 1–2 个环的唯一帧**
   （实测：轮期 17.5s→可见 11.8%、45.1s→6.3%、75.2s→**5.1%**，**轮越长越瞎**）。
   周期改 1s 后：**1 秒内渲染的帧数 ≤ 刷新率 × 1s < 环深 127 ⇒ 环装得下 ⇒ 溢出结构上不可能**
   ⇒ disjoint 归零、每轮环数 ＝ 轮期 ÷ 周期。
   ⇒ **只要周期 < 环深 ÷ 该批实测刷新率，轮期多长都不再影响 C 侧可见比例。**
   ⚠ **别把这个界写成常数（D-600）**：首窗五格的 `sf_latency` 头行报的是 **16.667ms（60Hz）**
   ⇒ 界＝ 2.12s；**而 P40 Pro 可跑 90Hz**，那时界只有 `127 ÷ 90 ≈ 1.41s`，**1s 周期的裕量从 2.1× 掉到 1.4×**。
   **`e2_precheck` 是按每格自己的头行算的（`ring_depth × period_ns`），不写死帧率**——
   照它印的建议值改即可；**要引数字就连「该批刷新率」一起引。**
   **别把「缩短周期」和「每轮多 dump 几次」当成两件事去排——后者正是前者的效果。**
   ⚠ **改前先核 adb 争用**：通道 C 的 dump 与通道 B 的 `screencap` 走同一条 adb，
   周期从 20s 提到 1s 是 20 倍的往返增量。**改一个采集参数前，先列出还有谁在用同一条通道。**
   ⚠ **反向的坑（采集侧实测自捉）**：为压 A 侧伪影而加的**静置期会拉长轮期**，
   在**未改周期**的前提下**把 C 侧变得更瞎**——**一个消费方的修法是另一个的退化。**
4. **双峰用单变量实验 settle**：发送后 **60 秒内不做任何操作**，看第二簇是否仍在 ~15.7s 形成。
   不导航就没有第二簇 ⇒ 导航是成因；照样有 ⇒ 是真的。**它是单变量的**，比「换答窗重测」干净
   （那次一改两个量，阴性结果无法归因）。
5. **F5/F6 各跑一格找窄带的正例**：现有边界描述**只有反例**，有正例才立得住。
6. **F1 换一条不指定长度的提示词**：当前那条把「一句话」写死在工作负载里，
   而结论会被读成整个功能的性质。

> **元规则（D-595②⑥）**：每批开跑前把**报告命题写死**成一张单子——
> 模板见 `docs/BATCH_PROPOSITION_SHEET_TEMPLATE.md`，复制为
> `docs/BATCH_PROPOSITION_<批次ID>.md`，**开窗前**填完并锁定（主命题此后只减不增）。
> 上面六条都是「怎么采」，而首窗真正的教训是**采完了答不上立项时想问的问题**——
> 命题不先写死，采集参数调得再对也救不回来。
> 该单的 §3 就是上面第 2、3 条的执行面（`e2_precheck` 与 dump 周期作为**开窗前置**），
> §4 则强制写下**本批不回答什么**——不写的话读者会把**范围外**读成**阴性结果**。
...
——它是**上行诉求的唯一证据源**；该格缺 `pcapdroid/` ⇒ **该功能的上行结论不给**。
🔴 **本窗（DW-20260830-01）通道 D 已降为可选、F3/F4 未跑**（原因＝自主模式 UI 导航
未验证，**不是能力否定**）⇒ **上行诉求维度本批整体缺失**。判读页必须**显式写出**
「本批不回答上行诉求」，不能只是不提它。
**「没测」与「没诉求」是两件事，而在报告里长得一模一样**——省略会被读成否定，
而这一维恰是本战役立项要答的问题之一。**未跑的功能一律带原因记 NOT_EXECUTED**：
「未跑」与「跑了但缺证据」处置不同（前者补跑即可，后者要查采集链）。
（不是「按其它通道估一个」）。
...
🔴 **但 `cadence_p50_ms` 不是逐格/逐轮的量，别当成那样递出去**：它与 `events`
都是**会话累计**、全程不重置，故 cadence 是**累计事件集上的滑动中位数**。
**流式一来就把它拖低**——采集侧窗内实测同一会话 5 秒之隔 **99.4 → 23.8**
（+39 个流式事件后，同机同 UI 栈）。我用归档语料独立验过累计性：一个会话内
`events` 单调 5→8→11→14→17→20。
⇒ **跨格比 cadence ＝ 比两个混了不同流式量的累计中位数**，不可比；要用它
必须锚在**固定读取点**（如「豆包切前台后的第一条 `ADAPTER_OBS`」——UI 栈判据
正是这么定的，晚读一条就会误判，而那是判据被稀释、不是 UI 栈变了）。
⚠ **别拿归档 E1 数据校准对它的直觉**：那是均匀 1.8s 刺激，同一会话内几乎不漂
（1806.9→1809.8）——**漂移幅度取决于负载**，均匀负载下的稳定是假象。
...
**🔴 判读第 −1 步：确认整批已收窗，否则一个数都不要下判断。**
**判据＝该批 `README.md` 存在**（收窗产物），**不是**某个格的文件存在。
**为什么不能用单文件**：重跑写回同名目录，而 `adapter.log` 以 `"wb"` **覆盖**打开、
`collect_notes.json` 也会被重新生成 ⇒ **「收尾」这个信号本身会被重置**。
**在一个会被原地重写的目录上，没有任何单文件能当定型信号。**
（2026-08-30 实证：我在窗内对同两个目录量了两次，一分钟内结果互相矛盾——
一格从「14 条标记」变成全空、另一格从 0 变成 609 事件。**我上一版把判据定成
「`collect_notes.json` 存在＝收尾」，当场被实况证伪。**）
⇒ **窗内看到的任何数都只能用来提醒采集方「去看一眼」，不得进任何结论**；
而**提醒本身要说清它是快照**——我曾据一个窗内快照建议作废一格真数据。

**⚠ 标记在不在，与标记能不能用，是两件事**（读码得出，与任何快照无关）：
`parse_marks` 在 `fit_wall_to_boot` 不 PASS 时**一条标记都不返回**（docstring 逐字：
「没有换算依据时给出的标记时刻是编的」），而 `fit` 的失败条件之一是**带 `t_boot_ns`
的 `ADAPTER_EVT` 行 < 2**。⇒ **一格可以「标记按齐了」而 e2 仍走 whole-run、n=1。**
收窗后逐格跑一次（几秒）：

```
python -c "import sys,os;sys.path.insert(0,'tools/e234');import e234_common as ec,e234_session as es;p=sys.argv[1];L=ec.read_lines(p,'adapter.log');f=ec.fit_wall_to_boot(L);print('fit=%s marks=%d'%(f.get('status'),len(es.parse_marks(L,f))))" <格目录>
```

`fit` 非 PASS 或 `marks=0` ⇒ 该格 e2 退化为 n=1，**该格不给跨通道自洽性结论**。
...
写「`<时间窗>` 内 `<功能>` 首帧中位 X ms」，**不得**写成「豆包 `<功能>` 需要 X ms」。
（示例原用 F3 上行——**该功能本窗未跑、上行维度亦缺**，照抄会写出一个本批
根本没有的数；示例正是被照抄的那半。）
...
（`e1_analyze.py:510`）。**这里有两句，别混成一句**——
**机制（工具强制）**：样本 `<2` 直接 `NOT_EXECUTED`，理由逐字「样本 <2，无法构成周期」
（`e1_analyze.py:504-505`）。**判断（我的，非机制）**：**建议 ≥3**——2 个样本只给出
1 个差值，分不出「一次性抖动」与「真实变化」，且翻转须恰好落在那两点之间才看得见。
**照抄成「工具要求 ≥3」就是造了一个不存在的机制断言**（v2 2026-08-30 核出，我原文
把判断紧挨着代码行号写，读起来像机制）。
首格跑完数 `screencap_index.jsonl` 在该轮 `s`(回答开始)→`a`(回答完成) 之间的行数：
⚠ **自动化模式下该阈值要抬到 5**：`s`/`a` 在自动化模式精度为 **±3s**
（`t` 轮界照常，不受影响）。周期 3000ms 下，**每个边界 ±1 样本、两端合计最坏 ±2**——
而阈值卡在 3，**边界误差足够把它推过线**。故：**手动模式 ≥3，自动化模式 ≥5**。
判读页须记本批是哪种模式，否则这个阈值无从解释。
**⚠ 这两个数的性质不同，别一视同仁**（判据承 v2 2026-08-30：*一个估算若其用法
本身就会当场检验它，其不确定性可控；危险的是用完就进结论、无人回头看的那种*）：
· **周期 ≈1.5s ⇒ 会被本批校正**——阈值的用法就是在首格**数真实样本数**，
  周期估错当场显形，最坏结果是按阶梯回退一次，**不会变成静默的错结论**；
· **`s`/`a` 的 ±3s ⇒ 本批不校正它**——没有任何环节会回头验它。它只决定
  **裕度**（3→5 那 2 个样本），故它若偏乐观，**后果是边缘格被误判为可用**，
  而不是把结论算错。**知道一个数不会被检验，比标它是估算更有用。**
⚠ **本行改过一次，原因值得记**：D-587 初版把该精度写作 ±1.5s，我据此定成「≥4」；
大脑随后把 ±1.5s 裁为笔误性过期数、实为 ±3s，阈值遂改 5。
**一个建立在别处给的数上的判断，那个数一变它就悄悄失准，而它不会自己报错**
——所以这类阈值必须把**它依赖的那个数**一起写出来，让下一个人改数时看得见它。
**<3 即回退 2000**，仍 <3 回退到 1500（＝不再减负，等同现状）。
**阶梯只在首格标定一次、全批统一，不得逐格重标**——逐格重标会让**通道 B 的
分辨率变成逐格变量**，跨格比 B 就成了拿不同采样率的数对比。首格若是流式最短的
那个功能（本窗 F1，实测仅 **7.9s**），它定出的值对其余功能自然够用，方向是安全的。
**选定值必须写进该批 README**——它是判读输入，且随机器/链路变（见上）。

**🔴 `NOT_EXECUTED` 有三种，读者会读成同一种，必须逐条写明原因**：
①**未跑**（该功能本窗没做）→ 补跑即可；
②**跑了但缺证据**（如该格无 `pcapdroid/`）→ 要查采集链；
③**跑了、证据齐、但工具对该形状结构上不适用** → **加轮数、补证据都没用**。
**第三种最容易被误读成前两种**，而它的含义是「这个问题在这个功能上问不了」。

**③的本批实例：`e2_analyze` 对 F1 结构性不适用**（采集侧两格实证 `a84803a`）。
e2 的 A2 锚点靠**渲染间隙**把帧序列切成两簇（门限 400ms，见下）。
⚠ **本条初版写成「F1 结构上没有思考停顿、每轮都分不出两簇」——逐轮数据推翻了它**：
6 轮格的 C 簇数是 `2,2,2,1,1,3`、5 轮格是 `2,2,2,1,2` ⇒ **多数轮拿得到 2 簇**，
丢的是**约 20–33% 的轮次**。且**丢弃轮与成功轮帧数相同（127/126）**
——**不是采少了，是那 127 帧里没有出现 > 400ms 的间隙**。
⇒ 正确说法：**不是「工具对 F1 不适用」，是「本批轮数预算买不到 n≥5」**（约需 8 轮）。
**判读处置**：F1 的 e2 判词记 `NOT_EXECUTED`，原因写「**可用轮 n=4 < 5：约 1/4–1/3
轮次的帧序列内无 > 400ms 渲染间隙**」——这是**可复核、且下批用轮数解决得了**的表述，
**不得写成执行失败、不得读成「F1 有问题」、也不得读成「e2 对 F1 永远不适用」**——F1 的量在通道 A 的 TTFT/cadence
与通道 B，那两项**不依赖 A2 锚点**，照常给。
⚠ **F1 是锚格功能** ⇒ **本批的漂移判据没有 e2 这一层**，判读页要显式写出来，
别让读者以为锚格被 e2 印证过。**🔴 `e2` 只在一条窄带里成立——这是仪器性质，不是某个功能的毛病**（F1/F2 两格实证）：
同一个 400ms 间隙门限**同时服务两条节奏差一个量级的流**——A 是 **~100ms 事件流**、
C 是 **~167ms 帧流**。于是两种失败模式**方向相反**：
· **短答（F1，流式 7.9s）**：A 簇 2–4 ✅、**C 簇常为 1** ❌ → 丢「通道 C 不足两簇」，丢 20–33%；
· **长答（F2，EVT 2451＝F1 的 3.4 倍）**：C 簇 2–4 ✅、**A 簇 6 轮里 4 轮为 1** ❌
  → 丢弃原因**翻转**成「通道 A 不足两簇」，丢 **67%**（长而连续的流式让事件密到无 400ms 间隙）。
**e2 要求同一轮里 A 与 C 都 ≥2 簇**，而 F1/F2 恰好落在这条窄带的两侧。

**⇒ 轮数这个旋钮只对 C 侧失败有效**：F1 排 8 轮可解；**F2 不可解**——**再多轮也是同样的
长答**，要 n≥5 约需 15 轮且是等比例烧额度。**判词必须按功能分开写**：F1 写「可用轮不足、
下批排 8 轮」（**可解**）；F2 写「**该负载下 A 侧无第二簇，`e2` 命题不成立**」，
**别让人去排 15 轮**。

**⇒ 不给 A/C 各自门限**——理由**不是**「两把尺子相减」（代码里 `gap` 取一次、两侧共用，
构造上已是一把尺，该担心可排除），而是更强的一条：**A2 是一个语义事件**（思考静默后
答案恢复的那一刻）。**两侧用不同判据，就可能各自指认了不同的事件**，`t_A − t_C` 于是
变成两件事之差——**那比精度问题严重，是命题被换掉了**。

**⇒ 「e2 适用带很窄」本身就是本批的一个产出**，要写进报告，不要只当失败记账：
它是关于**仪器**的实测结论（跨通道自洽性只在中等长度且带停顿的负载上可得），
比 e2 本来会给出的那几个数更耐用。

**簇分割门限不调（v3 裁，2026-08-30）**：`ec.cluster_gap_nanos()` **从设备端
`ObsStats.kt` 的 `CLUSTER_GAP_NANOS` 正则取值**（400ms，D-52/D-53），并在 e2 里
**同时喂给 A 侧与 C 侧** —— 两把尺子是被**设计成同一把**的。三条理由：
①它不是分析侧参数，改分析侧就与设备自报的 `ttft_cluster_ms` 用了不同尺子；
②改设备侧要重装探针，而**窗内重装会让全部前置核查作废**；
③400ms 是标定值，改它等于改「渲染间隙」在全仓的含义与历史可比性。
**要更高的 e2 可用率，正确旋钮是轮数（下批 F1 排 8 轮），不是门限、不是采样周期。**

e2 的适用性该到 **F2（深度思考）** 上验——
它是本批唯一**设计上带思考静默**的功能。

**并订正一条我自己的推荐的适用范围**：我建议过「每格多跑 1 轮买容错」（36 而非 30）。
**那条只在丢弃是随机时成立**；F1 这里丢弃是**结构性**的，加轮数买不到容错。
采集侧一度把首格 `dropped=1` 当作该建议的兑现，**6 轮实测把那个归因推翻了**
（真实丢弃率约 1/3，不是 1/5）。**建议本身不错，错的是拿一个结构性案例去证它。**

**⚠ 逐功能横向比较里含「提示词」这个我们自己定的变量**（采集侧 `5a1d052` 实测）：
F1 实测流式仅 **7.9 秒**，而 §10.2 估 30s——**高估约 4 倍，根因在提示词**：
F1 问的是「What is 5G in **one sentence**」，估算却按 ≈300 token 算。
⇒ **F1 的数刻画的是「一句话回答」的诉求，不是「文本对话」一般情形**；
写结论必须带**提示词口径**，与带时间窗同等要求。
⇒ 且**功能间的差异里含提示词选择带来的那部分**——F2 要深度思考、F1 只要一句话，
**「F2 比 F1 吃网络」可能只是「F2 的提示词要求更长回答」**。横向比较要么同工作负载，
要么把提示词差异写在结论旁边；**不写就是把我们自己的选择说成了产品的性质**。

**⚠ 通道 B 看不看得见「动」，取决于 ROI 怎么挑**（同笔实测）：全屏均值差仅 **0.72**，
远低于 `flip_threshold=8.0`；扫网格后唯一过阈的是一个小块（差 9.00）。
**大 ROI 会把变化平均掉** ⇒ **B 的空结果不能直接读成「没有画面在动」**，
先看该批 README 记的 ROI 是哪一块。**没记 ROI 的批，B 的阴性结论一律不给。**
