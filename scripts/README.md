...
本节以下累计 120+ 行、十余条口径，**当天逐段读来不及**。顺序即执行顺序：

| # | 做什么 | 不做会怎样 |
|---|---|---|
| 0 | 查战役 README 五条硬依赖（每格时间戳／切换时戳／每功能提示词逐字／锚格对照结论／ROI+两帧自检均值）；缺项在判读页首段写明「本批缺 X，故 Y 类结论不给」 | 结论没有支撑，且事后补不回来 |
| 1 | 核**条件是否齐**：只拿到单条件 ⇒ **删掉条件对照段**改绝对值登记 | 拿锚格/历史数凑跨条件结论 |
