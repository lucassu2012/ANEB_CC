...
### `check_evidence.py` — `evidence/` 规则的机器执行面（T82 §9.2 #4/#6/#7/#13/#14）

`evidence/README.md` 立了六条规则而此前**一条都没有守卫**。本脚本接上五条
（PASS 必须有证据／STATUS.json 四态与必填字段／日期包自带 README／命名禁区
`build`／日志 utf-8），外加一条 README 蕴含未写死的（列出的证据文件必须真在
盘上——「有证据」而文件名悬空，在读者眼里长得像合规）。**四态判据从 README
解析**，不在脚本里再抄一份；README 改到解析不出来时**报错而非放行**
（exit 2＝没法判，1＝判了有问题，0＝干净）。历史违规冻结成显式清单、
**只能缩不能长**：清偿后那条会变「过期豁免」反过来推红，逼人删掉它。
豁免**有归属**——换一棵树扫，它既不赦免谁也不宣告谁还清了。
...
## 观察批判读 · 一页速查（T+1 当天照此顺序做；细节见下文各节）

本节以下条目已多到当天逐段读来不及。顺序即执行顺序（细节见下文对应小节）：

| # | 做什么 | 不做会怎样 |
|---|---|---|
| 0 | 查战役 README 的**硬依赖项**（清单与理由见下文「判读第 0 步」；条目数随采集协议演进，**此处不复述数量**）；缺项在判读页首段写明「本批缺 X，故 Y 类结论不给」 | 结论没有支撑，且事后补不回来 |
| 1 | 核**条件是否齐**：只拿到单条件 ⇒ **删掉条件对照段**改绝对值登记，**且每个绝对值带时间窗** | 拿锚格/历史数凑跨条件结论；或写出无时段的绝对断言 |
| 1a | 核 `collect_notes.json` 的 **`sf_status`**：若为 `NOT_EXECUTED`（SurfaceFlinger 未找到该包图层）⇒ 该格**通道 C 为空，`e2_analyze` 会把该格每一轮都丢掉** | 跑完 e2 才发现整格无输出，而那时人已离开设备 |
| 1b | 核**该格有没有被补跑过**：`mark_rtt.jsonl` 的 `t` 数与 `adapter.log` 的轮数**对不上** ⇒ 当场回问采集侧，不按任一侧裁 | 按 t 切轮次切在空气上（见下「补跑格」段） |
| 2 | 核**制式**：每格前后两次 `gsm.network.type`；不一致 ⇒ 该格条件结论不给 | 「蜂窝 vs WiFi」里混进「NR vs LTE」（D-393 复发） |
| 3 | 读**锚格 vs 块 1**，先写这一段，再写条件对照 | 拿未经检验的前提去支撑结论 |
| 4 | 逐格跑 `e2_analyze`（`--run-dir` 指到**格目录**） | — |
| 5 | 取 `rule_matched` 等量时**先做差**（会话内累计，末行=该格合计） | 5 轮读成 1 轮 |
| 6 | 遇 `roi_mean=0.0` **先查该格是否息屏**，别判「ROI 作废」 | 方向完全反 |
| 7 | 缺 `pcapdroid/` ⇒ **F3/F4 上行结论不给** | 用别的通道估一个不存在的量 |
| 8 | 引用代码级语义时写明「据源码 @提交，装机构建 <版本>/<日期>」 | 源码与二进制不同源时，读数看起来完全正常 |
| 9 | 措辞：只写「前后两点实测同制式，格内瞬时漂移不可排除」，**不写「全程 NR」**；两个 `ttft_*` 写明用的哪个口径 | 说满了被推翻 |
...
**为什么只点 `e2_analyze` 一只**（采集侧板面提到「三只 analyze」，此处收窄）
——⚠ **本段依据是读 CLI 与 docstring，不是实测**：初版这里写的是「实测收窄」，
而下面给的理由全是源码阅读；采集侧照此转述成「按 v3 实测收窄」，等于给我的
源码阅读盖了实测的章。**措辞的强度会被转述放大，所以强度得由源头写准。**
...
**⚠ 通道 C 在 P40 上不是不能出，但四跑里只有一跑真出了量**（2026-08-29 实查归档
e234 真机跑）：`20260802-173031` 得 390 行 `sf_latency.txt`、`-172614` 只有 10 行、
`-163504`／`-164148` **整份缺失**，而后两次采集器自己记了原因——
`collect_notes.json` 的 `sf_status` ＝「`NOT_EXECUTED: SurfaceFlinger --list 未找到
该包的图层`」。即**根因是取样那一刻目标 App 没有图层（未前台/未绘制），不是设备
出不了帧**；它**在采集当时就可判**，不必等判读。

**这条对判读的意义**：`e2_analyze` 的「通道 C」读的是 `sf_latency.txt`
（`tools/e234/e2_analyze.py:57`），**不是** `framestats.txt`；该文件为空或不足即
`c_clusters < 2`，该轮记 `drop_reasons`「通道 C 该轮不足两簇」被丢弃。
**降级路径**：跨通道自洽性是 e2 的定义本身（拿通道 A 的 A2 与通道 C 的 A2 求差），
通道 C 没了就**没有「只用 A 通道」的等价物**——如实写「本格无跨通道自洽性结论」，
改出**逐通道各自的量**（A：事件流与 cadence；B：`roi_mean` 帧差），
⚠ **但 B 只够答粗问题**：采集侧 2026-08-30 实测（`2279d33`）通道 B 的实际
采样周期 ≈**1.5s**（`screencap` 单帧中位 1544ms，采样器「补睡 period−已耗时」
在 period=400 时恒为负 ⇒ 实际周期＝max(400, 单帧耗时)），而锚定 TTFT ≈2.0s
——**量具分辨率是被测量的约 77%**。⚠ **1.5s 是量级不是常数**：它与机器/USB
链路有关，协议要求**开窗当场重测**、且开 scrcpy 镜像后**再测一次**（镜像与
通道 B 走同一条显示捕获路径）。**判读时用该批 README 记的实测值，不要用这里
这个数**——这里给它只为说明「与 TTFT 同量级」这件事。故 B 在本批**只够回答「这一格到底有没有
画面在动」，不够回答「首帧早了/晚了多少」**（口径以采集侧那笔为准，此处不复述其推导）。
**降级时把 B 的数原样给出而不标这条边界，等于递给读者一个看着像测量、
却答不了该问题的量**——比不给更糟，因为它会被当成证据。
**不要把单通道读数说成对拍结果**。
...
**⚠ 降级成「绝对值登记」时，每个绝对值必须带时间窗**（口径见采集侧协议 §3，
`4ef9cb4` 定案；此处只写判读侧动作，不复述其论证）。本批是**单窗两小时**，
故 `ttft_ui_ms`／上传耗时／上行字节速率这类**绝对值只对该时段成立**：
写「`<时间窗>` 内 F3 上行中位 X Mbps」，**不得**写成「豆包 F3 需要 X Mbps 上行」。
**对照类结论不受此限**（WiFi vs 蜂窝、功能间相对差）——两条件同窗交替采，
时段对两边同等作用，这正是分块交替＋锚格买来的东西。
**注意这与 D-393 是反方向的同一个错**：D-393 是把时段当成了原因，
这里是把只在一个时段测到的绝对值当成与时段无关的常量；两个方向都出在
**没把时段写进结论**。降级路径尤其危险——它天生只产绝对值，而首段那句
免责只说了「条件」，说不到「时段」。

**⚠ 补跑格：`mark_rtt.jsonl` 是追加的，别把它的 `t` 数当轮数**（2026-08-29 实查
`tools/e234/e234_collect.py:228` `"a"`；文件名见 `:287`）。同格的
`adapter.log`／`screencap_index.jsonl`／`collect_notes.json` 是**覆盖**
（`:279`/`:359`/`:380`），而 `--out` 已存在时 `makedirs(exist_ok=True)` 静默复用。
**后果**：某格作废后在窗内补跑、写回同名目录 ⇒ marker 记「作废＋补跑」、
通道数据只记「补跑」，**两者不可能对齐且没有一处会报错**。另 `self.n` 每次进程
从 0 重起（`:227`），故同文件内 `n` 会重复。

**判读处置**：①以 `adapter.log` 的轮数为准，**不以 `t` 数为准**；
②两者不一致就**当场回问采集侧**（属承诺的三种回问情形），不自行按任一侧裁——
差额可能是补跑，也可能是漏按，**两者处置相反**；③采集侧已定案（协议 §④-A′，`f377c84`）：作废目录改名 `<格名>_VOID<k>/` 留着不删、
补跑写进干净的 `<格名>/`，故 `<格名>/` 可直接用，`_VOID*` **只进额度口径、
不进任何 n**。**但仍按 ① 核**——规则落地不等于每格都照做了，而漏改名与漏按键
在产物上长得一样。④`n` 从高位跌回 1 处是进程边界，可事后切分——**补救判据，不是设计**。

**⚠ 锚格劣化先怀疑热累积，别先归因网络/时段**（采集侧 `2279d33`／`4d2f85d` 实测：
抓屏每格 ≈7.6GB 传输、持续吃 1.3 核、cluster 57°C）。按「未测的不确定度三问」：
该代价**单向为正**、在**条件对照里两边同等抵消**，**唯一不抵消的是热累积随批次渐变**
——而那恰是末尾锚格要检测的东西。**故锚格与块 1 同格出现显著劣化时，第一嫌疑是
热累积/持续抓屏，不是网络也不是时段。** 与下面「块 1 vs 块 4 不一致 ⇒ 条件与时段
不可分离」是同表邻居，但**归因方向不同**：那条讲不可分离，这条讲有一个已知的、
由我们自己的仪器引入的渐变项。

**采样周期裁定（本参数属判读侧，v3 定，2026-08-30）**：`--screencap-period-ms`
**取 3000**。理由：①400 **已经买不到分辨率**——实际周期＝`max(period, 单帧耗时)`
而单帧 ≈1.5s，400 与 1500 等效；②3000 使负载减半，**减的正是上面那个唯一不抵消的
渐变项**，护的是通道 A 这个主量；③B 在本批只答「有没有画面在动」，粗粒度够用。
**判据与回退（必须在首格当场核，不能等窗后）**：帧差是**相邻两样本相减**
（`e1_analyze.py:510`），故窗内需 **≥3 个样本**才稳（2 个只有翻转恰好落在其间才看得见）。
首格跑完数 `screencap_index.jsonl` 在该轮 `s`(回答开始)→`a`(回答完成) 之间的行数：
**<3 即回退 2000**，仍 <3 回退到 1500（＝不再减负，等同现状）。
**选定值必须写进该批 README**——它是判读输入，且随机器/链路变（见上）。
