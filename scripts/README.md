...
| 3a | **先跑 `tools/e234/e2_precheck.py`（`--run-dir` 指到**格目录**），再决定这一格要不要跑 e2**。三态：`FAIL`＝该 App 结构性不适用（加轮数不解）；`NOT_EXECUTED`＝**装置没看见**（记录太碎／两通道矛盾）；`PASS`＝值得跑。**后两者绝不可互代** | 把「采集排期的洞」读成「App 的思考静默」——两者在去重后的帧序列里**长得一模一样**，而 e2 会照样出一个数 |
