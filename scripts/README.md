...
e2 的 A2 锚点靠**思考静默**把帧序列切成两簇，而 **F1 的提示词写着「in one sentence」
——它设计上就没有思考停顿**，约 1/3 轮次分不出第二簇（5 轮 dropped=1、6 轮 dropped=2，
同一条原因）。⇒ **加轮数只是线性缓解一个结构性不匹配**（要 n≥5 约需 8 轮且不稳）。
**判读处置**：F1 的 e2 判词记 `NOT_EXECUTED`，原因写「**结构性无第二簇**」，
**不得写成执行失败、也不得读成「F1 有问题」**——F1 的量在通道 A 的 TTFT/cadence
与通道 B，那两项**不依赖 A2 锚点**，照常给。
⚠ **F1 是锚格功能** ⇒ **本批的漂移判据没有 e2 这一层**，判读页要显式写出来，
别让读者以为锚格被 e2 印证过。e2 的适用性应到 **F2（深度思考）** 上验——
它是本批唯一**设计上带思考静默**的功能。
