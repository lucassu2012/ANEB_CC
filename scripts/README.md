...
⚠ **自动化模式下该阈值要抬到 4**（D-587）：`s`/`a` 在自动化模式是 **±1.5s 低精度**
（`t` 轮界照常）。周期 3000ms 下，**每个边界 ±0.5 样本、两端合计最坏 ±1**——
而阈值恰好卡在 3，**边界误差正好够把它推过线**。故：**手动模式 ≥3，自动化模式 ≥4**
（同样的安全裕度，抵掉那 ±1）。判读页须记本批是哪种模式，否则这个阈值无从解释。
