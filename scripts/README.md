...
故战役 README 必填项第 11 条记 `versionName` 与 `lastUpdateTime`，**让语料可归属到一个具体二进制**；
判读页引用代码级语义时须写明「据源码 @<提交>，装机构建 <versionName>/<日期>，
**未逐字节验证同源**」。**若两者不同源而我按源码语义读数，读出来的东西看起来完全正常**
——这正是本节其它假阳性（`roi_mean=0.0`）的同族：错法不报错，只给出一个像样的数。
...
写清各格实际制式（取每格 `radio` 证据或 README 记载），制式不一致的格**单列不并入**，
