...
> 输入记录：20；含 run.aqs：20；含 campaign 标签：20。样本地板 min_samples=5。

## 覆盖盘点

- 战役 campaign_id：{'m3-expansion-wave0': 20}
- 点位 point_id：{'SZ-PILOT-01': 20}
- 运营商 carrier：{'ctcc': 20}
- 时段 time_band：{'idle': 20}
- 服务层级 tier：{'metro': 20}
- run 状态 status：{'completed': 20}
- profile 版本：{'s1_chat@0.2.1;s2_coding_agent@0.2.1;s3_multimodal@0.3.0': 20}
- 标签来源 label_source：{'set': 20}
- 采集时间窗：2026-08-03 16:14 UTC → 2026-08-03 18:20 UTC

## 溯源 / provenance（可复现性）

> 工具 `aneb-campaign-analysis/1.0` · 生成 2026-08-04 02:34:30 +0800 · 读 20 行 → 保留 20 条（去重丢 0）。参数 {"min_samples": 5, "attr_kpi": "n1_rtt_p50_ms", "campaign": null, "before": null, "after": null}。
...
| 输入文件 | sha256 |
|---|---|
| wave0_counted_labelled.jsonl | `4f02344e724f…` |
...
- **体验最差格**：1 个格中无 fair/poor（最低 SZ-PILOT-01/ctcc/idle=86.8）；且这 **20 条 run 全部被打分器自评低置信**（`run.aqs.low_confidence`，热力卡 `SCORER_LOW_CONF`）——**分数自己声明了不确定**，本行的分级不得当作定论。
- **分段归因**（`n1_rtt_p50_ms`；主要贡献段）：接入 3 格；最大单项 SZ-PILOT-01/ctcc/idle/s2_coding_agent·接入=77.8ms；各段跨单元离差不可比较（可比单元不足）。
- **批化失真热点**：1 个 —— SZ-PILOT-01/ctcc/idle。
- **时钟可疑热点**：无。
- **有效率**：全部达门（≥80%）。
- **复测不稳定**：5/9 单元超 CV 门 —— SZ-PILOT-01/ctcc/idle/metro/s3_multimodal·t1_ttft_ms、SZ-PILOT-01/ctcc/idle/metro/s1_chat·t1_ttft_ms、SZ-PILOT-01/ctcc/idle/metro/s1_chat·n1_rtt_p50_ms 等 5 个。
- **疑似序位偏倚**：3/9 处位置-KPI 相关 —— s1_chat/n1_rtt_p50_ms（极差 25.7%）、s3_multimodal/n1_rtt_p50_ms（极差 21.7%）、s2_coding_agent/n1_rtt_p50_ms（极差 10.5%）（反平衡可能失效，**本报告的 KPI 中位数据此存疑**）。
- **疑似预热效应**：1/3 个 KPI 首轮系统性更差 —— n1_rtt_p50_ms（首轮劣 18.1%）（首轮读数偏保守；**跨格比较不受影响**，每格一样冷）。
- **无线上下文**：1 个格均有可用无线证据（可据此排除信号因素）。
- **接入介质**：无同格双介质可比，或蜂窝不劣于 wifi。
- **分数侧归因**（拖累维度）：N1 1 格；最低 SZ-PILOT-01/ctcc/idle·N1=64.2。
...
|---|---|---|---|---|---|---|---|
| SZ-PILOT-01 | ctcc | idle | 86.8 | 0.8 | excellent | 20 | SCORER_LOW_CONF:20/20; MIXED_MODE:forensic/quick |
...
|---|---|---|---|---|---|---|---|
| SZ-PILOT-01 | ctcc | idle | 60.43 | 50.49–65.63（3 个 profile） | excellent | 90 | — |
...
| 点位 | 运营商 | 时段 | 中位 | profile 跨度 | 分级 | n | 备注 |
|---|---|---|---|---|---|---|---|
| SZ-PILOT-01 | ctcc | idle | 76.29 | 73.52–77.84（3 个 profile） | fair | 90 | — |
...
| 点位 | 运营商 | 时段 | 中位 | profile 跨度 | 分级 | n | 备注 |
|---|---|---|---|---|---|---|---|
| SZ-PILOT-01 | ctcc | idle | 4.81 | 4.49–5.11（2 个 profile） | fair | 60 | RULED_OUT:s1_chat×30（D-366） |
...
| 点位 | 运营商 | 时段 | 中位 | profile 跨度 | 分级 | n | 备注 |
|---|---|---|---|---|---|---|---|
| SZ-PILOT-01 | ctcc | idle | 39.59 | 21.09–39.82（3 个 profile） | excellent | 90 | — |
...
> **结论**：3/3 个单元在当前 n 下**没有 80% 的把握**看见 5% 的差异；这些单元的建议复测数中位为 **n≥85**（每侧）。 其中 **3 个**单元的当前 n 恰好落在「差异等于噪声尺度」附近——**那只有约五成把握**，不要据此认为采样量已经够了。
...
> **结论**：3/3 个单元在当前 n 下**没有 80% 的把握**看见 5% 的差异；这些单元的建议复测数中位为 **n≥45**（每侧）。 其中 **3 个**单元的当前 n 恰好落在「差异等于噪声尺度」附近——**那只有约五成把握**，不要据此认为采样量已经够了。
...
> **结论**：1/3 个单元在当前 n 下**没有 80% 的把握**看见 5% 的差异；这些单元的建议复测数中位为 **n≥41**（每侧）。 其中 **1 个**单元的当前 n 恰好落在「差异等于噪声尺度」附近——**那只有约五成把握**，不要据此认为采样量已经够了。
...
|---|---|---|---|---|---|---|
| s1_chat | #0:50.5(n=20) / #5:50.2(n=5) / #7:51.1(n=5) | 0.9 | 1.8 | 50.5 | 无明显效应 | — |
| s2_coding_agent | #1:65.8251(n=20) / #3:64.1315(n=5) / #8:66.2794(n=5) | 2.1479 | 3.2728 | 65.6288 | 无明显效应 | — |
| s3_multimodal | #2:61.6(n=20) / #4:63.2(n=5) / #6:64.7(n=5) | 3.1 | 5 | 62.4 | 无明显效应 | — |
...
| profile | 位次中位数 | 极差 | 极差% | 总体中位 | 判定 | 备注 |
|---|---|---|---|---|---|---|
| s1_chat | #0:79.5(n=20) / #5:71.3(n=5) / #7:60.6(n=5) | 18.9 | 25.7 | 73.5 | **疑似序位偏倚** | — |
| s2_coding_agent | #1:81.48(n=20) / #3:73.34(n=5) / #8:73.61(n=5) | 8.14 | 10.46 | 77.84 | **疑似序位偏倚** | — |
| s3_multimodal | #2:79.17(n=20) / #4:73.51(n=5) / #6:62.4(n=5) | 16.77 | 21.74 | 77.13 | **疑似序位偏倚** | — |
...
| profile | 位次中位数 | 极差 | 极差% | 总体中位 | 判定 | 备注 |
|---|---|---|---|---|---|---|
| s1_chat | #0:0.12508(n=20) / #5:0.12899(n=5) / #7:0.136418(n=5) | 0.011338 | 8.666434 | 0.130822 | 无明显效应 | ROUNDING_UNRECONCILED |
| s2_coding_agent | #1:4.539412(n=20) / #3:4.367247(n=5) / #8:4.524806(n=5) | 0.172165 | 3.830563 | 4.494497 | 无明显效应 | ROUNDING_UNRECONCILED |
| s3_multimodal | #2:5.13315(n=20) / #4:5.09407(n=5) / #6:5.03642(n=5) | 0.09673 | 1.89327 | 5.10915 | 无明显效应 | — |
...
|---|---|---|---|---|
| t1_ttft_ms | #0:60.8(n=60) / #1:60.7(n=15) / #2:59.75(n=15) | 1 | 无明显预热 | — |
| n1_rtt_p50_ms | #0:79.75(n=60) / #1:73.25(n=15) / #2:61.86(n=15) | 18.1 | **疑似预热效应** | — |
| u1_goodput_mbps | #0:4.8(n=40) / #1:4.71(n=10) / #2:4.84(n=10) | -0.5 | 无明显预热 | RULED_OUT:30（D-366） |

## 有效性与失效原因（每格的有效样本分母）

> 全语料尝试 90 个场景，可用 90 （100.0%）；低于 80% 的单元标 `LOW_VALID_RATE`。INVALID 场景 KPI 为空、会被热力卡/归因静默丢弃——**此表即那些被丢弃样本的去向**。
...
|---|---|---|---|---|---|---|---|---|---|---|
| SZ-PILOT-01 | ctcc | idle | s1_chat | 30 | 0 | 30 | 0 | 0 | 100.0% | — |
| SZ-PILOT-01 | ctcc | idle | s2_coding_agent | 30 | 0 | 30 | 0 | 0 | 100.0% | — |
| SZ-PILOT-01 | ctcc | idle | s3_multimodal | 30 | 0 | 30 | 0 | 0 | 100.0% | — |
...
|---|---|---|---|---|---|---|---|---|---|
| SZ-PILOT-01 | ctcc | idle | 90 | 90 | 33 (37%) | 70.9 | 0/90 | 141.4 | — |
...
| N2 | 90 | 0 (0%) | 17 |
| T2 | 90 | 0 (0%) | 397 |
| T2_incl_coalesced | 90 | 0 (0%) | 397 |
| T3 | 90 | 0 (0%) | 397 |
| T3_incl_resume | 90 | 0 (0%) | 397 |
| T4 | 90 | 0 (0%) | 397 |
| T5 | 90 | 0 (0%) | 0 |
| U1_excl_slow_start | 90 | 0 (0%) | 0 |
| U2 | 90 | 0 (0%) | 0 |
...
|---|---|---|---|---|---|---|
| point_id=SZ-PILOT-01 · carrier=ctcc · time_band=idle · profile_id=s1_chat | 同城 | 73.5 | — | — | — | TIER_MISSING:regional,core; MIXED_MODE:forensic/quick |
| point_id=SZ-PILOT-01 · carrier=ctcc · time_band=idle · profile_id=s2_coding_agent | 同城 | 77.8 | — | — | — | TIER_MISSING:regional,core; MIXED_MODE:forensic/quick |
| point_id=SZ-PILOT-01 · carrier=ctcc · time_band=idle · profile_id=s3_multimodal | 同城 | 77.1 | — | — | — | TIER_MISSING:regional,core; MIXED_MODE:forensic/quick |
...
|---|---|---|---|---|---|---|---|
| 接入(metro) | 3 | 77.1 | 0.7 | 0.9% | — | — | 可比单元 <4，**在申明口径下无法筛查**（该规模下没有任何阈值能把误报压到目标）→ 需要更多点位，不是更多复测 |
...
| 单元 | 覆盖层级 | 接入(metro) | 区域骨干+ | 核心骨干+ | 端到端(core) | 备注 |
|---|---|---|---|---|---|---|
| point_id=SZ-PILOT-01 · carrier=ctcc · time_band=idle · profile_id=s1_chat | 同城 | 50.5 | — | — | — | TIER_MISSING:regional,core; MIXED_MODE:forensic/quick |
| point_id=SZ-PILOT-01 · carrier=ctcc · time_band=idle · profile_id=s2_coding_agent | 同城 | 65.6 | — | — | — | TIER_MISSING:regional,core; MIXED_MODE:forensic/quick |
| point_id=SZ-PILOT-01 · carrier=ctcc · time_band=idle · profile_id=s3_multimodal | 同城 | 62.4 | — | — | — | TIER_MISSING:regional,core; MIXED_MODE:forensic/quick |
...
| 段 | 参与单元 | 典型值(中位) | 离差(MAD) | 离差/典型 | 偏高 | 偏低 | 判读 |
|---|---|---|---|---|---|---|---|
| 接入(metro) | 3 | 62.372 | 3.257 | 5.222% | — | — | 可比单元 <4，**在申明口径下无法筛查**（该规模下没有任何阈值能把误报压到目标）→ 需要更多点位，不是更多复测 |
| 区域骨干+ | 0（另 3 不可计算） | — | — | — | — | — | 可比单元不足(<2)，无法比较 |
| 核心骨干+ | 0（另 3 不可计算） | — | — | — | — | — | 可比单元不足(<2)，无法比较 |
...
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| SZ-PILOT-01 | ctcc | idle | 20 | 95.1 | 96.8 | 100 | 64.2 | 81.3 | 70.1 | 80.8 | **N1**=64.2 | 35.8 | — |
...
|---|---|---|---|---|---|---|---|---|---|---|---|
| SZ-PILOT-01 | ctcc | idle | 90 | 0 | 599 | none | 0.257 | 0.396 | 0 | 56% | **失真热点** |
...
|---|---|---|---|---|---|---|---|---|
| SZ-PILOT-01 | ctcc | idle | — | 86.8 (n=20) | — | — | — | — |
...
|---|---|---|---|---|---|---|---|---|---|
| SZ-PILOT-01 | ctcc | idle | 中 | -98 | 16.5 | NR | 1 个 | 106.88.83.76 | — |
