...
| `refresh_hz` | 90.000 |
| `screen_px` | 1200x2486 |
| 翻转总数 / 可用（去预热、有 commit） | 77 / 76 |

**一帧 = 16.667 ms**（实测，非硬编码 33ms —— spec §3.1）。

## 1. 时钟基（跨通道比较的前提）

BOOTTIME − MONOTONIC 偏移中位数 = 94513649504729 ns，跨度 = 4168 ns（n=76）。
...
| C 渲染时间线 | t_commit → t_present（同基） | 53 | 23 | 16.512 | 28.748 | 29.427 | **FAIL** — p99 29.427ms > 1 帧 16.667ms |
| B screencap 帧差 | **不报时间误差**，只报采样周期 | 20 | — | 3050.987 | 4829.546 | 5089.520 | PASS |
...
通道 B 检出翻转 0 次（刺激源共翻 77 次）——检出率不是时序主张，只说明 ROI 与阈值选得对不对。
