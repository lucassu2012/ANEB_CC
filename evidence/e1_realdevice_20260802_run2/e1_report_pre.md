...
| `refresh_hz` | 90.000 |
| `screen_px` | 1200x2486 |
| 翻转总数 / 可用（去预热、有 commit） | 60 / 59 |
| ⚠ 重复 seq（App 被重启过，后者覆盖前者） | 1 |

**一帧 = 11.111 ms**（实测，非硬编码 33ms —— spec §3.1）。

## 1. 时钟基（跨通道比较的前提）

BOOTTIME − MONOTONIC 偏移中位数 = 94513649504207 ns，跨度 = 4168 ns（n=59）。
...
| C 渲染时间线 | t_commit → t_present（同基） | 0 | 59 | — | — | — | **NOT_EXECUTED** — 缺分布或缺实测刷新率 |
...
通道 B 检出翻转 3 次（刺激源共翻 60 次）——检出率不是时序主张，只说明 ROI 与阈值选得对不对。
