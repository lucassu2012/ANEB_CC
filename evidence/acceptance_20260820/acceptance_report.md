...
> 输入记录：10；含 run.aqs：10；含 campaign 标签：10。样本地板 min_samples=5。

## 覆盖盘点

- 战役 campaign_id：{'acceptance_20260820': 10}
- 点位 point_id：{'home_indoor': 10}
- 运营商 carrier：{'中国电信': 10}
- 时段 time_band：{'idle': 10}
- 服务层级 tier：{'unknown': 10}
- run 状态 status：{'completed': 10}
- profile 版本：{'s1_chat@0.2.1;s2_coding_agent@0.2.1;s3_multimodal@0.3.0': 10}
- 标签来源 label_source：{'set+inferred:time_band(tz=+8)': 10}
- 采集时间窗：2026-08-19 23:18 UTC → 2026-08-19 23:47 UTC

## 溯源 / provenance（可复现性）

> 工具 `aneb-campaign-analysis/1.0` · 生成 2026-08-20 07:57:07 +0800 · 读 10 行 → 保留 10 条（去重丢 0）。参数 {"min_samples": 5, "attr_kpi": "n1_rtt_p50_ms", "campaign": null, "before": null, "after": null}。
...
| 输入文件 | sha256 |
|---|---|
| acceptance_20260820_labeled.jsonl | `277fd57f7fe5…` |


> ⚠ **`time_band` 有 10/10 条是工具推断的**（按 `started_at_epoch_ms` 的本地小时,非现场记录;规则与所用时区偏移见「覆盖盘点」的 `label_source`）。忙闲差异的结论**须注明这一点**——推断错时段会把两类流量混在一起,而表面上看不出来。

## 摘要（先看这里）

> 下列每条中的示例均为该项**最严重的前三个**（其余以「等 N 个」计数，完整清单见对应段落与 CSV）。

- **体验最差格**：1 个格中无 fair/poor（最低 home_indoor/中国电信/idle=89）；且这 **10 条 run 全部被打分器自评低置信**（`run.aqs.low_confidence`，热力卡 `SCORER_LOW_CONF`）——**分数自己声明了不确定**，本行的分级不得当作定论。
- **分段归因**：无可归因单元（`n1_rtt_p50_ms` 缺数据或缺层级）。
- **批化失真**：无热点格。
- **时钟可疑热点**：无。
- **有效率**：全部达门（≥80%）。
- **复测不稳定**：9/9 单元超 CV 门 —— home_indoor/中国电信/idle/unlabeled/s3_multimodal·t1_ttft_ms、home_indoor/中国电信/idle/unlabeled/s2_coding_agent·t1_ttft_ms、home_indoor/中国电信/idle/unlabeled/s1_chat·t1_ttft_ms 等 9 个。
...
- **无线上下文**：全部无线样本标 stale 已排除——**排除不等于没问题**，须核对采集侧的取样新鲜度窗口。
- **接入介质**：无同格双介质可比，或蜂窝不劣于 wifi。
- **分数侧归因**（拖累维度）：N1 1 格；最低 home_indoor/中国电信/idle·N1=62.6。
...
|---|---|---|---|---|---|---|---|
| home_indoor | 中国电信 | idle | 89 | 1 | excellent | 10 | SCORER_LOW_CONF:10/10; **MIXED_TRANSPORT:cellular/wifi** |
...
|---|---|---|---|---|---|---|---|
| home_indoor | 中国电信 | idle | 67.42 | 67.42–68.42（3 个 profile） | excellent | 30 | — |
...
| 点位 | 运营商 | 时段 | 中位 | profile 跨度 | 分级 | n | 备注 |
|---|---|---|---|---|---|---|---|
| home_indoor | 中国电信 | idle | 77.51 | 76.26–79.73（3 个 profile） | fair | 30 | — |
...
| 点位 | 运营商 | 时段 | 中位 | profile 跨度 | 分级 | n | 备注 |
|---|---|---|---|---|---|---|---|
| home_indoor | 中国电信 | idle | 13.48 | 10.69–17.61（2 个 profile） | good | 20 | RULED_OUT:s1_chat×10（D-366） |
...
| 点位 | 运营商 | 时段 | 中位 | profile 跨度 | 分级 | n | 备注 |
|---|---|---|---|---|---|---|---|
| home_indoor | 中国电信 | idle | 26.67 | 12.99–28.58（3 个 profile） | excellent | 30 | — |
...
| 单元 | n | 中位 | 均值 | CV% | 稳定? | 备注 |
|---|---|---|---|---|---|---|
| campaign_id=acceptance_20260820 · point_id=home_indoor · carrier=中国电信 · time_band=idle · tier=unlabeled · profile_id=s1_chat | 10 | 68.19 | 67.23 | 21 | ✗超门 | — |
| campaign_id=acceptance_20260820 · point_id=home_indoor · carrier=中国电信 · time_band=idle · tier=unlabeled · profile_id=s2_coding_agent | 10 | 67.42 | 67.69 | 25 | ✗超门 | — |
| campaign_id=acceptance_20260820 · point_id=home_indoor · carrier=中国电信 · time_band=idle · tier=unlabeled · profile_id=s3_multimodal | 10 | 68.42 | 66.61 | 25.8 | ✗超门 | — |
...
| 单元 | n | 中位 | 均值 | CV% | 稳定? | 备注 |
|---|---|---|---|---|---|---|
| campaign_id=acceptance_20260820 · point_id=home_indoor · carrier=中国电信 · time_band=idle · tier=unlabeled · profile_id=s1_chat | 10 | 79.73 | 80.07 | 17.4 | ✗超门 | — |
| campaign_id=acceptance_20260820 · point_id=home_indoor · carrier=中国电信 · time_band=idle · tier=unlabeled · profile_id=s2_coding_agent | 10 | 76.26 | 78.73 | 18.6 | ✗超门 | — |
| campaign_id=acceptance_20260820 · point_id=home_indoor · carrier=中国电信 · time_band=idle · tier=unlabeled · profile_id=s3_multimodal | 10 | 76.68 | 76.89 | 18.5 | ✗超门 | — |
...
| 单元 | n | 中位 | 均值 | CV% | 稳定? | 备注 |
|---|---|---|---|---|---|---|
| campaign_id=acceptance_20260820 · point_id=home_indoor · carrier=中国电信 · time_band=idle · tier=unlabeled · profile_id=s1_chat | 10 | 0.12 | 0.12 | 18.9 | ✗超门 | — |
| campaign_id=acceptance_20260820 · point_id=home_indoor · carrier=中国电信 · time_band=idle · tier=unlabeled · profile_id=s2_coding_agent | 10 | 10.69 | 10.46 | 15.8 | ✗超门 | — |
| campaign_id=acceptance_20260820 · point_id=home_indoor · carrier=中国电信 · time_band=idle · tier=unlabeled · profile_id=s3_multimodal | 10 | 17.61 | 17.85 | 14 | ✗超门 | — |
...
|---|---|---|---|---|---|---|---|---|---|---|
| campaign_id=acceptance_20260820 · point_id=home_indoor · carrier=中国电信 · time_band=idle · tier=unlabeled · profile_id=s1_chat | 10 | 68.19 | 21 | **✗超门** | 7.9 | 11.6% | 14.55 | ✗不足 | 54 | 183 |
| campaign_id=acceptance_20260820 · point_id=home_indoor · carrier=中国电信 · time_band=idle · tier=unlabeled · profile_id=s2_coding_agent | 10 | 67.42 | 25 | **✗超门** | 9.5 | 14.1% | 17.5 | ✗不足 | 80 | 270 |
| campaign_id=acceptance_20260820 · point_id=home_indoor · carrier=中国电信 · time_band=idle · tier=unlabeled · profile_id=s3_multimodal | 10 | 68.42 | 25.8 | **✗超门** | 9.61 | 14% | 17.7 | ✗不足 | 79 | 268 |

> **结论**：3/3 个单元在当前 n 下**没有 80% 的把握**看见 5% 的差异；这些单元的建议复测数中位为 **n≥268**（每侧）。
...
| 单元 | n | 中位 | CV% | 超门? | 可辨最小差异(平) | 占中位(平) | 可辨最小差异(80%) | 达标?(80%) | 需 n≥(平) | 需 n≥(80%) |
|---|---|---|---|---|---|---|---|---|---|---|
| campaign_id=acceptance_20260820 · point_id=home_indoor · carrier=中国电信 · time_band=idle · tier=unlabeled · profile_id=s1_chat | 10 | 79.73 | 17.4 | **✗超门** | 7.82 | 9.8% | 14.4 | ✗不足 | 39 | 131 |
| campaign_id=acceptance_20260820 · point_id=home_indoor · carrier=中国电信 · time_band=idle · tier=unlabeled · profile_id=s2_coding_agent | 10 | 76.26 | 18.6 | **✗超门** | 8.22 | 10.8% | 15.14 | ✗不足 | 47 | 158 |
| campaign_id=acceptance_20260820 · point_id=home_indoor · carrier=中国电信 · time_band=idle · tier=unlabeled · profile_id=s3_multimodal | 10 | 76.68 | 18.5 | **✗超门** | 7.99 | 10.4% | 14.71 | ✗不足 | 44 | 148 |

> **结论**：3/3 个单元在当前 n 下**没有 80% 的把握**看见 5% 的差异；这些单元的建议复测数中位为 **n≥148**（每侧）。
...
| 单元 | n | 中位 | CV% | 超门? | 可辨最小差异(平) | 占中位(平) | 可辨最小差异(80%) | 达标?(80%) | 需 n≥(平) | 需 n≥(80%) |
|---|---|---|---|---|---|---|---|---|---|---|
| campaign_id=acceptance_20260820 · point_id=home_indoor · carrier=中国电信 · time_band=idle · tier=unlabeled · profile_id=s1_chat | 10 | 0.12 | 18.9 | **✗超门** | 0.01 | 10.8% | 0.02 | ✗不足 | 47 | 158 |
| campaign_id=acceptance_20260820 · point_id=home_indoor · carrier=中国电信 · time_band=idle · tier=unlabeled · profile_id=s2_coding_agent | 10 | 10.69 | 15.8 | **✗超门** | 0.92 | 8.6% | 1.7 | ✗不足 | 30 | 102 |
| campaign_id=acceptance_20260820 · point_id=home_indoor · carrier=中国电信 · time_band=idle · tier=unlabeled · profile_id=s3_multimodal | 10 | 17.61 | 14 | **✗超门** | 1.4 | 8% | 2.59 | ✗不足 | 26 | 87 |

> **结论**：3/3 个单元在当前 n 下**没有 80% 的把握**看见 5% 的差异；这些单元的建议复测数中位为 **n≥102**（每侧）。
...
|---|---|---|---|---|---|---|
| s1_chat | #0:68.2(n=10) | — | — | 68.2 | 不可计算 | NEED_2_POSITIONS; low_conf |
| s2_coding_agent | #1:67.4(n=10) | — | — | 67.4 | 不可计算 | NEED_2_POSITIONS; low_conf |
| s3_multimodal | #2:68.4(n=10) | — | — | 68.4 | 不可计算 | NEED_2_POSITIONS; low_conf |
...
| profile | 位次中位数 | 极差 | 极差% | 总体中位 | 判定 | 备注 |
|---|---|---|---|---|---|---|
| s1_chat | #0:79.7(n=10) | — | — | 79.7 | 不可计算 | NEED_2_POSITIONS; low_conf |
| s2_coding_agent | #1:76.3(n=10) | — | — | 76.3 | 不可计算 | NEED_2_POSITIONS; low_conf |
| s3_multimodal | #2:76.7(n=10) | — | — | 76.7 | 不可计算 | NEED_2_POSITIONS; low_conf |
...
| profile | 位次中位数 | 极差 | 极差% | 总体中位 | 判定 | 备注 |
|---|---|---|---|---|---|---|
| s1_chat | #0:0.1(n=10) | — | — | 0.1 | 不可计算 | NEED_2_POSITIONS; low_conf |
| s2_coding_agent | #1:10.7(n=10) | — | — | 10.7 | 不可计算 | NEED_2_POSITIONS; low_conf |
| s3_multimodal | #2:17.6(n=10) | — | — | 17.6 | 不可计算 | NEED_2_POSITIONS; low_conf |
...
> 全语料尝试 30 个场景，可用 30 （100.0%）；低于 80% 的单元标 `LOW_VALID_RATE`。INVALID 场景 KPI 为空、会被热力卡/归因静默丢弃——**此表即那些被丢弃样本的去向**。
...
|---|---|---|---|---|---|---|---|---|---|---|
| home_indoor | 中国电信 | idle | s1_chat | 10 | 0 | 10 | 0 | 0 | 100.0% | — |
| home_indoor | 中国电信 | idle | s2_coding_agent | 10 | 0 | 10 | 0 | 0 | 100.0% | — |
| home_indoor | 中国电信 | idle | s3_multimodal | 10 | 0 | 10 | 0 | 0 | 100.0% | — |
...
|---|---|---|---|---|---|---|---|---|---|
| home_indoor | 中国电信 | idle | 30 | 30 | 5 (17%) | 43.5 | 0/30 | 154.7 | — |
...
| U1 | 30 | 30 (100%) | 1 |
| D1 | 30 | 10 (33%) | 0 |
| U1_excl_slow_start | 30 | 10 (33%) | 0 |
| D3 | 30 | 0 (0%) | 0 |
| D3_excl_slow_start | 30 | 0 (0%) | 0 |
| N1 | 30 | 0 (0%) | 17 |
| N2 | 30 | 0 (0%) | 17 |
| T2 | 30 | 0 (0%) | 395 |
| T2_incl_coalesced | 30 | 0 (0%) | 397 |
| T3 | 30 | 0 (0%) | 395 |
| T3_incl_resume | 30 | 0 (0%) | 395 |
| T4 | 30 | 0 (0%) | 395 |
| T5 | 30 | 0 (0%) | 0 |
| U2 | 30 | 0 (0%) | 0 |
| U3 | 30 | 0 (0%) | 0 |
| U3_excl_slow_start | 30 | 0 (0%) | 0 |
...
> ⚠ coverage：10 条记录无 tier 标签，未进归因。

_无可归因单元（记录均缺 tier 标签或缺该 KPI）。_
...
|---|---|---|---|---|---|---|---|
| 接入(metro) | 0 | — | — | — | — | — | 可比单元不足(<2)，无法比较 |
| 区域骨干+ | 0 | — | — | — | — | — | 可比单元不足(<2)，无法比较 |
| 核心骨干+ | 0 | — | — | — | — | — | 可比单元不足(<2)，无法比较 |
...
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| home_indoor | 中国电信 | idle | 10 | 94.9 | 98.1 | 100 | 62.6 | 87.2 | 82.6 | 79.7 | **N1**=62.6 | 37.4 | — |
...
|---|---|---|---|---|---|---|---|---|---|---|---|
| home_indoor | 中国电信 | idle | 30 | 0 | 599 | none | 0.146 | 0.02 | 0.005 | 33% | — |
...
|---|---|---|---|---|---|---|---|---|
| home_indoor | 中国电信 | idle | 88.36 (n=5) | 89.93 (n=5) | 1.57 | ±0.3 | — | — |

## 无线上下文（信号档与小区一致性）

_本轮所有无线样本均标 `stale`,已全部排除——**排除不等于没问题**,也不等于信号良好;须核对采集侧的取样新鲜度窗口。_

## 纵向趋势

> **本段无数据，不是「全部平稳」。** 本语料有 **1 个带标签战役**，而趋势需要至少 **3** 个才能表达轨迹——两个只够说前后（见「优化前后对比」段），第三轮起本段自动出现。

html -> evidence/acceptance_20260820/acceptance_report.html
