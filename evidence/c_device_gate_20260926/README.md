...
> **现态（2026-09-26T14:14:29 run6 复跑）：门通**——见第 9 节。本节以下是 12 时的诊断结论，保留原样（含订正）。

**三层门＝不通（12 时）。** 手机 ping 223.5.5.5 在两个时刻都没收到回复（#20 回复 0/3；#38 回复 0/3）。
...
- gate_diag6.py 是第 8 节预登记的 run6；它写 readings_run6.json（`--selftest` 只写 readings_run6_selftest.json，不碰设备）。

## 8. run6 预登记（大脑 2026-09-26 批准；跑前写死、随脚本一起提交，跑后不改）

**做什么**：run5 原样（字段名、adb 命令块、判「通」的方法一字不改），外加两样：PC 热点卡的**单播**计数；与每个 ping 窗**等长**的背景窗（ping 窗一结束立刻测，时长取该 ping 窗的实测时长），两窗读数并印在同一条记录里。脚本 gate_diag6.py。跑的时点由大脑在 PO 报「已亮屏」后下发。

**计数器与通道**：iphlpapi `GetIfEntry2` 返回的 `MIB_IF_ROW2.OutUcastPkts`（单播发送）、`OutDiscards`（发送丢弃）、`InUcastPkts`（单播接收），python ctypes 直调，读 ULONG64 原值，函数返回码逐次记为 `uc_rc`（0＝NO_ERROR）。接口号开跑时由 `GetIpAddrTable` 按热点地址现场推出，不写死。

**计数器自检**（脚本内写死，任一不过即视为「取不到」⇒ 不测背景窗、不出判词，只跑 run5 原样字段，**不凑替代量法**）：结构体大小等于 1352；`GetIfEntry2` 返回 0；别名与 psutil 持有热点地址的网卡同名；接口号一致；psutil 的发/收包数与字节数被 ctypes 前后两读夹住；单播发送计数非零。`--selftest` 另加一道独立来源：PowerShell `Get-NetAdapterStatistics` 的单播发/收计数被 ctypes 前后两读夹住。本次预登记前已跑过一次自检，结果原样在 readings_run6_selftest.json（看其 `verdict` 字段）。

**记号**：d ＝ ping 上游窗的单播发送增量 − 其背景窗的单播发送增量；丢弃 ＝ 同两窗 `OutDiscards` 增量之差。「回复数」＝上游对 3 个请求应回的回复数。

**世界表（预期）**——任两世界预期相同即本量法分不开：

| 世界 | d | 丢弃 | 手机 ping 上游 |
|---|---|---|---|
| H1 屏灭致下行不到（关联在、NAT 对） | ≈回复数 | ≈0 | 屏灭 0/3；亮屏后应通 |
| H2 共享源错，PC 对转发失败不回差错 | ≈0 | ≈0 | 0/3 |
| H2′ 共享源错，PC 对转发失败回 ICMP 差错 | ≈回复数 | ≈0 | 0/3（差错行不含来自目标的回复） |
| H3a 关联坏，驱动把发往手机的包照记为已发 | ≈回复数 | ≈0 | 0/3 |
| H3b 关联坏，驱动把发往手机的包记为丢弃 | ≈0 | ≈回复数 | 0/3 |

**判据**（脚本从下面这一行解析，这是唯一来源；解析不出脚本直接抛错）：

```
PREREG: replies=3; bg_max=2; d_reply=[2,4]; d_bg=[-1,1]; disc_reply=[2,4]; disc_zero=[0,1]
```

**判词映射**（脚本按此由读数算出判词文字，不写死结论）：背景窗单播发送增量 > bg_max ⇒ 判不了；d ∈ d_reply ⇒ 排除 H2（纯），剩 H1／H2′／H3a；d ∈ d_bg 且 丢弃 ∈ disc_zero ⇒ 支持 H2；d ∈ d_bg 且 丢弃 ∈ disc_reply ⇒ 支持 H3b；其余 ⇒ 判不了。手机若已收到回复（通），判词前加注「计数只作一致性核对」。

**已知盲区（跑前写明）**：H1、H2′、H3a 三者的 d 预期相同，本量法分不开——亮屏把 H1 分出去，H2′ 与 H3a 仍要靠第 1 节第 2 条（看共享源）。「回复数＝3」的前提是上游对 3 个请求都回复；本诊断里 PC 以太网作源 ping 上游两次都是 3/3，但那是 PC 本机的路径。ping 网关的窗照测、照印，但只作辅助，不出判词（PC 回不回 echo 未知，D-906）。

**跑前已知的世界变化（预登记时观察到，不改判据，只改归因与窗口有效性）**——该卡的统计在一小时内**至少被清零两次**：

| 时刻 | 来源 | 该卡单播发送计数 | 证据等级 |
|---|---|---|---|
| 12:37 | run4，PowerShell `Get-NetAdapterStatistics` | {'ifIndex': 13, 'ReceivedUnicastPackets': 416062, 'SentUnicastPackets': 866052} | 脚本自写文件 |
| 13:36 | run6 第一次自检，同一 PowerShell 计数器 | 2657（ctypes 前后两读 2656／2657） | ⚠ **转述**：这次自检的文件被 13:42 第二次自检以同一文件名覆盖，读数只剩执行窗会话记录里的原样输出；脚本已改为文件名带时刻 |
| 13:42 | run6 第二次自检，同一 PowerShell 计数器 | {'sent_ucast': [134, 138, 138], 'recv_ucast': [135, 138, 138]}（每组三个数＝ctypes 前读、PowerShell、ctypes 后读） | 脚本自写文件 readings_run6_selftest_134216.json |
| 13:44–13:47 | gate_watch.py 每秒一读，共 143 次 | 清零与链路态变化事件 0 次；首读 153、末读 162 | 脚本自写文件 readings_hotspot_watch_134410.json |

同次自检顺带读的 PC 侧热点现况：{'ts': '2026-09-26T13:42:00', 'rc': 0, 'State': 'On', 'Clients': 1, 'NicStatus': 'Up'}（WinRT 状态是全局的；接口号 13）。

- **推断（未证实）**：第二次清零的时刻只知道在 13:36 那次自检之后、13:42 那次之前；到 13:42 那一读（清零后至多约 6 分钟）计数已长到上表的量级，而 13:44–13:47 采样三分钟只长个位数 ⇒ 清零后的增长速率明显高于其后，像是清零时刻伴随一阵突发流量，例如客户端重新关联时热点虚拟卡被重建。若属实，它与 H3（关联坏）同族，但本诊断没有直接读数。
- **对 run6 的两条后果（已焊进脚本）**：①窗口内任一累计计数变小、或接口号变化 ⇒ 该窗作废，判词为「判不了（窗口内计数器被清零或接口号变化）」；②run5 与 run6 之间变的**不止「亮屏」一个量**——run6 若通，不能单归因于亮屏（H1），判词照算，但归因句必须带上这一条。
- 背景量级参考：上表采样三分钟里单播发送只长个位数，秒级窗口的背景远低于 bg_max；这是预登记时的一次观察，不是判据。

**追加（大脑 2026-09-26 14 时要求，跑前入库；只增加作废条件，不改 PREREG 行）**：

- **关联稳定性是一格读数**：每个 ping 窗开始前、其背景窗结束后，各读一次手机 supplicant 态与 wlan0 地址（`cmd wifi status` 只 grep 出 supplicant 态，输出不含网络名）、PC 虚拟卡链路态（GetIfEntry2 的 OperStatus/MediaConnectState）与接口号；任一项前后不一致，或手机不是 COMPLETED ⇒ 该窗作废，标「关联不稳」，**不记「不通」**。整轮前后各读一次热点客户端数（WinRT，全局态）。
- **输出文件名一律带开始时刻**（run6 与自检都是），不再互相覆盖。
- **14:02:15 第一次正式跑**（readings_run6_140215.json，脚本当时还没有上两条）：屏幕亮、未锁屏、前台华为桌面都对，但冲突门查到 ANEB 进程 ['com.aneb.experiencelab'] ⇒ **两格 ping 未执行**（CLAUDE.md P40 流程第 3 步：归属不明不覆盖）。该包的安装时间等见下一条快照；之后两次只读快照里进程已不在、不在无障碍服务名单、没有在跑的服务，**归属仍未知**。
- **两侧关联只读快照**（gate_assoc.py；readings_assoc_140712.json 是网络名解析失败的第一版，保留；readings_assoc_141001.json 是修正后）：PC 热点 On、客户端数 1；手机 supplicant COMPLETED、**当前网络名与 PC 热点网络名相同＝True**（名称只在内存比对，不打印不落盘，D-608⑤）；手机 wlan0 192.168.137.48/24（run3 时是 192.168.137.129 ⇒ 期间重新拿过租约，与计数清零同向）。BSSID 与手机网卡地址这台机对 shell 不给，两格记「未读到」。PO 截图的「已连接 0 台」与此不矛盾——截图时刻未知，关联在掉与重连之间。

## 9. run6 结果（readings_run6_141410.json；按第 8 节预登记与追加条件，脚本 gate_diag6.py 未在跑后改动）

| 读数 | 值 | 三态 |
|---|---|---|
| 冲突门（2026-09-26T14:14:11） | 屏幕 Awake、锁屏 false、前台 com.huawei.android.launcher/com.huawei.android.launcher.unihome.UniHomeLauncher、ANEB 进程 []、隧道 [] | 通 |
| **手机 ping 223.5.5.5（门）**，2026-09-26T14:14:29 | 发 3（`ping -c 3`），来自目标且带 ttl 的回复 3；ping 自身退出码 0；手机 wlan0 收包增量 3 | **通** |
| 同窗单播计数 | d＝3（ping 窗 3 − 背景窗 0），丢弃差 0；窗长 2.92 秒 | —（一致性核对） |
| 脚本算出的判词 | 手机已收到回复（通），计数只作一致性核对：d=3≈回复数：PC 向手机发出了回复 ⇒ 排除「纯 H2」（共享源错且 PC 不回差错） | — |
| 窗口有效性 | 关联前后变化项 []；计数清零 ping 窗 []、背景窗 []；热点客户端数整轮前后变化＝False | 有效 |
| 手机 ping 网关（辅助，不作门），2026-09-26T14:14:18 | 发 3，回复 0；PC 热点卡单播接收 +3、单播发送 +0（背景窗发送 +1） | 不通（不作判据） |
| 屏幕态（收尾） | Awake | — |

- **门通，时刻限定**：以上是 2026-09-26T14:14:29 这一刻的往返；本诊断同一小时里见过关联掉线重连，通不代表稳定。
- **归因：不单归 H1。** run5（12:44）到 run6 之间至少变了四个量：①屏幕 Asleep → Awake；②关联重建（手机 wlan0 地址 run3 的 192.168.137.129 → 现在的 {'ts': '2026-09-26T14:14:29', 'supplicant': 'COMPLETED', 'inet': '192.168.137.48/24', 'ifindex': [13], 'pc_link': {'rc': 0, 'oper': 1, 'media': 1}} 中的 inet，热点卡统计清零至少两次）；③时间约一个半小时；④PO 查看过共享源（称未改）。**能说的**：此刻纯 H2 被 3/3 与 d≈回复数排除；共享源＝以太网由 PO 截图证实，但那排的是配置面，不是转发路径（D-906 分寸）。**不能说的**：run5 的不通是屏灭（H1）还是关联坏后已自愈（H3）——两者在 run6 这一刻读数相同。
- **网关 ping 的新读数**：同窗手机发出 3 个、PC 热点卡收到 3 个单播，而 PC 在该窗没有发出任何单播（背景窗反而 +1）⇒ PC 不回 echo 这件事**有了直接读数**，不再只是 D-906 里「可以合法不回」的推断（「收到的 3 个就是 echo 请求」是由同窗手机发包数推的）；网关 ping 不作门再次得到支持。
- **局限**：ping 的原始输出没有落盘（只落了回复计数与 ping 自身退出码），run5 同。本脚本不在跑后改动；若再跑，须先改脚本落原样输出，改动同样跑前入库。
- **P40 流程收尾**：本跑没有启动或停止任何 App，没有改任何设置；屏幕是 PO 点亮的，收尾时仍是 Awake，与开跑时一致。
