...
**一个混杂量没有排除：全程手机屏灭锁屏**（mWakefulness=Asleep、锁屏显示中，#16、#36、#39）。下行不到可能是屏灭省电，也可能是关联本身坏了——本诊断**区分不了，不猜**。「下行不通」的确切含义也只到这一层：包**没进手机网络栈**（手机 wlan0 收包计数不动）；是空口就没送到，还是到了被手机网卡固件过滤掉，计数分不开。大脑那次 0/3 当时的屏幕态不知道。

**与 D-871 不是同一个故障。** D-871 是共享源选到了死卡：手机能连上、能拿到地址，但转发出不去。今天 PC 以热点地址为源发出的包能出去也能回来，而第一跳下行本身就不回，D-871 的修法（改共享源）解释不了这个读数。但共享源直读是「未执行」，所以「共享源选错」这一条**没有被排除**，只是不在主嫌位置。

⚠ 下面这条是推断，不是直读：以热点地址为源的回复 TTL 比以太网直发的回复**少 1**（逐条原样见表），说明回复多经过一次本机转发，推断 NAT 出口在以太网——若共享源是 WLAN，这个包从以太网出去时不会被 NAT，推断它回不来。这条推断只说明**在读数那一刻**以太网出口的 NAT 在工作，不能替代直读共享源。

## 1. 给 PO 的动作（每条一行，按顺序；本窗不代做）

1. **点亮 P40 屏幕（不必解锁），告诉执行窗 61bd2401**——我原样复跑 run5（约 15 秒、只读）。亮屏后通 ⇒ 屏灭下行问题；亮屏仍不通 ⇒ 做第 3 条。
2. **在 PC「设置 → 网络和 Internet → 移动热点 →『共享我的以下 Internet 连接』」看一眼选的是哪张卡**，告诉我；若是 WLAN，改成「以太网」（改设置由 PO 做）。
3. （仅当第 1 条亮屏后仍不通）**在 P40 上关开一次 WLAN、重连 PC 热点**（改设备状态，由 PO 做），然后我复跑。
...
- **计数增量的读法**：增量为 0 当强读数；增量 ≥3 混有背景流量，只当弱读数。run5 的 PC 侧计数是 psutil（GetIfEntry2，单播加非单播），紧贴 adb 调用前后读，窗口秒级。
