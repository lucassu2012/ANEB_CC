# ds_cell_f1 · DW-20260905-02（DeepSeek 四格批，大脑自开窗）

- 条件：cell；功能：F1；提示词：`What is 5G in one sentence`；轮数 6；答窗 45s ＋ 静置 20s（实际值见包级 `ds_cell_f1_driver_timing.jsonl` 逐轮）
- **开关态（D-641）**：深度思考OFF+智能搜索OFF（同上）；模式选择器=快速模式(默认未动)（截图自读确认，图未存档）
...
- 制式：pre=`NR_SA,Unknown` post=`NR_SA,Unknown`；路由 pre=`1.1.1.1 via 10.42.241.236 dev rmnet0 table 1008 src 10.42.241.236 uid 2000 ` post=`1.1.1.1 via 10.42.241.236 dev rmnet0 table 1008 src 10.42.241.236 uid 2000 `
