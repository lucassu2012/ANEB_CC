# verify_trial_f1 · DW-20260905-02（DeepSeek 四格批，大脑自开窗）

- 条件：wifi；功能：F1；提示词：`What is 5G in one sentence`；轮数 1；答窗 60s ＋ 静置 20s（实际值见包级 `verify_trial_f1_driver_timing.jsonl` 逐轮）
- **开关态（D-641）**：深度思考OFF+智能搜索OFF（冷启默认双ON，脚本钩子点 (220,2525)(545,2525) 关闭并像素核对）；模式选择器=快速模式(默认未动)（截图自读确认，图未存档）
