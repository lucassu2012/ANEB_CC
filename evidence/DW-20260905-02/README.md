...
- **⑥ P3 定性（承 B-11 `b9f4fc9`，D-737）**：「A 侧可用轮 0/12」是 **400ms 绝对簇阈的口径效应**——四格 24 轮 A 侧每轮 148–163 条内容事件、零事件 0 轮，k10/20/40 相对门限下 24/24 可切；但 **C 侧四种口径下 0/24 从不闭合**，e2 需两通道同时有 A2，故「e2 对 DeepSeek 不可用」仍是事实，理由订正为「A 侧静默尺度与 400ms 不匹配＋C 侧根本不静默」。机制：DeepSeek A 侧中位相邻间隔 0.1–0.2ms（流式逐字），豆包 99–109ms，差三个数量级；同一个 k 在两 App 上方向相反（换成相对门限豆包 7/14 会掉到 0/14）⇒ 生产口径不换。P3 判词因此写作：**口径内不同型，成因＝事件节拍差三个数量级（App 事件流形状的真实差异，不是网络差异，也不是「A 侧没数据」）**。
