...
- **②§1b 试水数订正（结论不变）**：F2 ＝ **1406 事件／整场 107.6s（窗内 1258／98.5s）**，F1 ＝ **232／9.8s（窗内 161／1.2s）**，F6 ＝ 617／18.9s（窗内 497／9.9s）。锁定正文里的 1396／32.0s 与 228／9.7s 是**采集器仍在写日志时提前读出的数**（末事件当时尚未落盘）。F6 对照真短答 F1 仍明显非短答拒绝，D-637② 分支不触发的结论不变；**污染面已封口（v4 扫同源量）**：ROI 均值极差 18.3／30.7／19.2 与三试水 P1 输出均与产物精确吻合，提前读只污染了那两个 A 侧计数（F6 的 617 是对的，因其事件流在读取时已流完）；但按窗内跨度 F6（9.9s）离 F1（1.2s）比离 F2（98.5s）更近，如实记。
...
- **⑤ P3 判词的口径边界（承 M-B-014①）**：「A 侧可用轮 0/12 vs 豆包 7/14」是 **400ms 绝对簇阈（`cluster_gap_nanos`）口径内**的判词；Compose 栈的 A 侧事件流上该阈可能切在分布尾巴，「切不出次簇」是口径效应还是事实，**待 B-11 `gap_compare.py`（k∈{10,20,40}×median 并排，不换口径）对本批四格＋豆包 F1 六格离线复算**（派 v3）；A-3 分流须把这 12 轮的「零事件」与「单簇」分开计数。在此之前 P3 只写「口径内不同型」，**不写成 App 差异**。
- **⑥ P3 定性（承 B-11 `b9f4fc9`，D-737）**：「A 侧可用轮 0/12」是 **400ms 绝对簇阈的口径效应**——四格 24 轮 A 侧内容事件总量 1422／1465／3377／3747（wifi_f1／cell_f1／cell_f6／wifi_f6）、零事件 0 轮（⚠ **勘误 D-749**：本行原写「每轮 148–163 条」，那是**单格极差被写成全批区间**，F6 两格六轮总量已否证它——若每轮≤163 则六轮上限仅 978），k10/20/40 相对门限下 24/24 可切；但 **C 侧四种口径下 0/24 从不闭合**，e2 需两通道同时有 A2，故「e2 对 DeepSeek 不可用」仍是事实，理由订正为「A 侧静默尺度与 400ms 不匹配＋C 侧根本不静默」。机制：DeepSeek A 侧中位相邻间隔 0.1–0.2ms（流式逐字），豆包 99–109ms，**实测比值约 177–1119 倍（≈2.65 个数量级）**（⚠ 勘误 D-749：原写「三个数量级」，是取两端极值相除且两区间本身偏窄）；同一个 k 在两 App 上方向相反（换成相对门限豆包 7/14 会掉到 0/14）⇒ 生产口径不换。P3 判词因此写作：**口径内不同型，成因＝事件节拍相差约两个半数量级（App 事件流形状的真实差异，不是网络差异，也不是「A 侧没数据」）**。
