# verify_trial_f6 · DW-20260905-02（DeepSeek 四格批，大脑自开窗）

- 条件：wifi；功能：F6；提示词：`Generate an image of a red circle on a white background.`；轮数 1；答窗 120s ＋ 静置 20s（实际值见包级 `verify_trial_f6_driver_timing.jsonl` 逐轮）
- **开关态（D-641）**：深度思考ON+智能搜索OFF；模式选择器=快速模式(默认未动)；确认=收起态芯片像素均值 思考(164,186,254) 搜索(158,158,158)+截图自读 00:18（截图自读确认，图未存档）
