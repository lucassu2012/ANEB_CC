...
> 输入记录：4；含 run.aqs：4；含 campaign 标签：4。样本地板 min_samples=5。

## 覆盖盘点

- 战役 campaign_id：{'m2-pilot-forensic-20260731': 4}
- 点位 point_id：{'SZ-PILOT-01': 4}
- 运营商 carrier：{'ctcc': 4}
- 时段 time_band：{'busy': 4}
- 服务层级 tier：{'metro': 4}
- run 状态 status：{'completed': 4}
- profile 版本：{'s1_chat@0.2.1;s2_coding_agent@0.2.1;s3_multimodal@0.3.0': 4}
- 标签来源 label_source：{'set+inferred:time_band(tz=+8)': 4}
- 采集时间窗：2026-07-31 06:58 UTC → 2026-07-31 07:37 UTC
...
| 输入文件 | sha256 |
|---|---|
| forensic_labelled.jsonl | `0e1a7f74c173…` |


> ⚠ **`time_band` 有 4/4 条是工具推断的**（按 `started_at_epoch_ms` 的本地小时,非现场记录;规则与所用时区偏移见「覆盖盘点」的 `label_source`）。忙闲差异的结论**须注明这一点**——推断错时段会把两类流量混在一起,而表面上看不出来。

## 摘要（先看这里）

> 下列每条中的示例均为该项**最严重的前三个**（其余以「等 N 个」计数，完整清单见对应段落与 CSV）。

- **体验最差格**：1 个格中无 fair/poor（最低 SZ-PILOT-01/ctcc/busy=90.6，n=4 low_conf）——但这 **1 个格全部 low_confidence**（每格 n<5），**本轮不足以判定好坏**，先把样本补够再读这一条；且这 **4 条 run 全部被打分器自评低置信**（`run.aqs.low_confidence`，热力卡 `SCORER_LOW_CONF`）——**分数自己声明了不确定**，本行的分级不得当作定论。
- **分段归因**（`n1_rtt_p50_ms`；主要贡献段）：接入 3 格；最大单项 SZ-PILOT-01/ctcc/busy/s2_coding_agent·接入=59.6ms；各段跨单元离差不可比较（可比单元不足）。
- **批化失真**：无热点格。
- **时钟可疑热点**：无。
- **有效率**：全部达门（≥80%）。
- **复测不稳定**：3/9 单元超 CV 门 —— SZ-PILOT-01/ctcc/busy/metro/s3_multimodal·t1_ttft_ms、SZ-PILOT-01/ctcc/busy/metro/s2_coding_agent·t1_ttft_ms、SZ-PILOT-01/ctcc/busy/metro/s2_coding_agent·u1_goodput_mbps。
- **疑似序位偏倚**：6/9 处位置-KPI 相关 —— s3_multimodal/t1_ttft_ms（极差 21.8%）、s2_coding_agent/n1_rtt_p50_ms（极差 18.9%）、s1_chat/t1_ttft_ms（极差 16.9%） 等 6 个（反平衡可能失效，**本报告的 KPI 中位数据此存疑**）。
- **疑似预热效应**：2/3 个 KPI 首轮系统性更差 —— n1_rtt_p50_ms（首轮劣 12.9%）、u1_goodput_mbps（首轮劣 11.6%）（首轮读数偏保守；**跨格比较不受影响**，每格一样冷）。
- **无线上下文**：本轮语料**完全没有**——**无从核对**结论里是否混着信号差异（**采集缺口，不是「信号良好」**；生产侧接线规格见 `docs/RADIO_CONTEXT_WIRING_SPEC.md`）。
- **接入介质**：无同格双介质可比，或蜂窝不劣于 wifi。
- **分数侧归因**（拖累维度）：N1 1 格；最低 SZ-PILOT-01/ctcc/busy·N1=73.2。
...
|---|---|---|---|---|---|---|---|
| SZ-PILOT-01 | ctcc | busy | 90.6 | 0.7 | excellent | 4 | SCORER_LOW_CONF:4/4; low_conf |
...
> **本表共 3 个单元**：✗超门 2，CV 不可计算 0，其余稳定。摘要的「N/M 单元超 CV 门」即各 KPI 分表这两个数各自相加。

| 单元 | n | 中位 | 均值 | CV% | 稳定? | 备注 |
|---|---|---|---|---|---|---|
| campaign_id=m2-pilot-forensic-20260731 · point_id=SZ-PILOT-01 · carrier=ctcc · time_band=busy · tier=metro · profile_id=s1_chat | 12 | 48.44 | 47.74 | 9.5 | 稳定 | — |
| campaign_id=m2-pilot-forensic-20260731 · point_id=SZ-PILOT-01 · carrier=ctcc · time_band=busy · tier=metro · profile_id=s2_coding_agent | 12 | 48.48 | 50.55 | 10.9 | ✗超门 | — |
| campaign_id=m2-pilot-forensic-20260731 · point_id=SZ-PILOT-01 · carrier=ctcc · time_band=busy · tier=metro · profile_id=s3_multimodal | 12 | 48.34 | 49.95 | 10.9 | ✗超门 | — |
...
| 单元 | n | 中位 | 均值 | CV% | 稳定? | 备注 |
|---|---|---|---|---|---|---|
| campaign_id=m2-pilot-forensic-20260731 · point_id=SZ-PILOT-01 · carrier=ctcc · time_band=busy · tier=metro · profile_id=s1_chat | 12 | 55.12 | 55.73 | 7.1 | 稳定 | — |
| campaign_id=m2-pilot-forensic-20260731 · point_id=SZ-PILOT-01 · carrier=ctcc · time_band=busy · tier=metro · profile_id=s2_coding_agent | 12 | 59.62 | 57.54 | 9.6 | 稳定 | — |
| campaign_id=m2-pilot-forensic-20260731 · point_id=SZ-PILOT-01 · carrier=ctcc · time_band=busy · tier=metro · profile_id=s3_multimodal | 12 | 58.31 | 57.36 | 6.4 | 稳定 | — |
...
| 单元 | n | 中位 | 均值 | CV% | 稳定? | 备注 |
|---|---|---|---|---|---|---|
| campaign_id=m2-pilot-forensic-20260731 · point_id=SZ-PILOT-01 · carrier=ctcc · time_band=busy · tier=metro · profile_id=s1_chat | 12 | 0.15 | 0.16 | 4.7 | 稳定 | — |
| campaign_id=m2-pilot-forensic-20260731 · point_id=SZ-PILOT-01 · carrier=ctcc · time_band=busy · tier=metro · profile_id=s2_coding_agent | 12 | 10.92 | 11.24 | 10.3 | ✗超门 | — |
| campaign_id=m2-pilot-forensic-20260731 · point_id=SZ-PILOT-01 · carrier=ctcc · time_band=busy · tier=metro · profile_id=s3_multimodal | 12 | 17.16 | 17.25 | 9.2 | 稳定 | — |

## 序位效应诊断（t1_ttft_ms；拉丁方反平衡是否奏效）

> 判据：同 profile 各**执行位次**的中位数极差 / 总体中位数 > 10.0% 即疑似残留序位偏倚（无效应=好结果）。

> 轮转口径：共 3 种轮次，其中 4 条 run **在自身内部**已轮转（`s1,s2,s3|s2,s3,s1` 形状），反平衡在构造上成立。

| profile | 位次中位数 | 极差 | 极差% | 总体中位 | 判定 | 备注 |
|---|---|---|---|---|---|---|
| s1_chat | #0:51.04(n=4) / #5:48.84(n=4) / #7:42.87(n=4) | 8.17 | 16.87 | 48.44 | **疑似序位偏倚** | low_conf |
| s2_coding_agent | #1:53.6(n=4) / #3:46.3(n=4) / #8:47.7(n=4) | 7.3 | 15.1 | 48.5 | **疑似序位偏倚** | low_conf |
| s3_multimodal | #2:57.234(n=4) / #4:46.716(n=4) / #6:46.822(n=4) | 10.518 | 21.761 | 48.335 | **疑似序位偏倚** | low_conf |
...
| profile | 位次中位数 | 极差 | 极差% | 总体中位 | 判定 | 备注 |
|---|---|---|---|---|---|---|
| s1_chat | #0:59.63(n=4) / #5:53.4(n=4) / #7:52.27(n=4) | 7.36 | 13.35 | 55.12 | **疑似序位偏倚** | low_conf |
| s2_coding_agent | #1:63.8245(n=4) / #3:52.54(n=4) / #8:56.055(n=4) | 11.2845 | 18.9261 | 59.624 | **疑似序位偏倚** | low_conf |
| s3_multimodal | #2:60.1(n=4) / #4:55.3(n=4) / #6:54.9(n=4) | 5.2 | 8.9 | 58.3 | 无明显效应 | low_conf |
...
| profile | 位次中位数 | 极差 | 极差% | 总体中位 | 判定 | 备注 |
|---|---|---|---|---|---|---|
| s1_chat | #0:0.151102(n=4) / #5:0.159576(n=4) / #7:0.154923(n=4) | 0.008474 | 5.468579 | 0.154955 | 无明显效应 | low_conf; ROUNDING_UNRECONCILED |
| s2_coding_agent | #1:10.26323(n=4) / #3:11.9571(n=4) / #8:11.25554(n=4) | 1.69387 | 15.51473 | 10.91782 | **疑似序位偏倚** | low_conf |
| s3_multimodal | #2:17.914223(n=4) / #4:17.688322(n=4) / #6:16.7857(n=4) | 1.128523 | 6.57756 | 17.157167 | 无明显效应 | low_conf |
...
|---|---|---|---|---|
| t1_ttft_ms | #0:51.75(n=12) / #1:47.35(n=12) / #2:47.13(n=12) | 9.5 | 无明显预热 | — |
| n1_rtt_p50_ms | #0:60.2(n=12) / #1:53.01(n=12) / #2:53.62(n=12) | 12.9 | **疑似预热效应** | — |
| u1_goodput_mbps | #0:10.26(n=12) / #1:11.96(n=12) / #2:11.26(n=12) | 11.6 | **疑似预热效应** | — |
...
|---|---|---|---|---|---|---|---|---|---|---|
| SZ-PILOT-01 | ctcc | busy | s1_chat | 12 | 0 | 12 | 0 | 0 | 100.0% | — |
| SZ-PILOT-01 | ctcc | busy | s2_coding_agent | 12 | 0 | 12 | 0 | 0 | 100.0% | — |
| SZ-PILOT-01 | ctcc | busy | s3_multimodal | 12 | 0 | 12 | 0 | 0 | 100.0% | — |
...
|---|---|---|---|---|---|---|---|---|---|
| SZ-PILOT-01 | ctcc | busy | 36 | 36 | 2 (6%) | 17.5 | 0/36 | 130.2 | — |
...
|---|---|---|---|---|---|---|
| point_id=SZ-PILOT-01 · carrier=ctcc · time_band=busy · profile_id=s1_chat | 同城 | 55.1 | — | — | — | TIER_MISSING:regional,core |
| point_id=SZ-PILOT-01 · carrier=ctcc · time_band=busy · profile_id=s2_coding_agent | 同城 | 59.6 | — | — | — | TIER_MISSING:regional,core |
| point_id=SZ-PILOT-01 · carrier=ctcc · time_band=busy · profile_id=s3_multimodal | 同城 | 58.3 | — | — | — | TIER_MISSING:regional,core |
...
| 单元 | 覆盖层级 | 接入(metro) | 区域骨干+ | 核心骨干+ | 端到端(core) | 备注 |
|---|---|---|---|---|---|---|
| point_id=SZ-PILOT-01 · carrier=ctcc · time_band=busy · profile_id=s1_chat | 同城 | 48.4 | — | — | — | TIER_MISSING:regional,core |
| point_id=SZ-PILOT-01 · carrier=ctcc · time_band=busy · profile_id=s2_coding_agent | 同城 | 48.5 | — | — | — | TIER_MISSING:regional,core |
| point_id=SZ-PILOT-01 · carrier=ctcc · time_band=busy · profile_id=s3_multimodal | 同城 | 48.3 | — | — | — | TIER_MISSING:regional,core |
...
| 段 | 参与单元 | 典型值(中位) | 离差(MAD) | 离差/典型 | 偏高 | 偏低 | 判读 |
|---|---|---|---|---|---|---|---|
| 接入(metro) | 3 | 48.442 | 0.041 | 0.085% | — | — | 可比单元 <4，**在申明口径下无法筛查**（该规模下没有任何阈值能把误报压到目标）→ 需要更多点位，不是更多复测 |
| 区域骨干+ | 0（另 3 不可计算） | — | — | — | — | — | 可比单元不足(<2)，无法比较 |
| 核心骨干+ | 0（另 3 不可计算） | — | — | — | — | — | 可比单元不足(<2)，无法比较 |
...
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| SZ-PILOT-01 | ctcc | busy | 4 | 96.4 | 98 | 100 | 73.2 | 86.9 | 82 | 85.4 | **N1**=73.2 | 26.8 | low_conf |
...
|---|---|---|---|---|---|---|---|---|---|---|---|
| SZ-PILOT-01 | ctcc | busy | 36 | 0 | 599 | none | 0.02 | 0.01 | 0 | 25% | — |
...
|---|---|---|---|---|---|---|---|---|
| SZ-PILOT-01 | ctcc | busy | — | 90.6 (n=4*) | — | — | — | — |
