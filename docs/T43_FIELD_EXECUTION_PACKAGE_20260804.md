...
## 4. 外场协作纪律：大脑逐 run 盯采（T44 工具就绪后生效）

**目标协作节奏**（大脑派单原话）："PO 采一个 run，大脑这边 T44 工具即时验一个，问题
当场重采不白跑。"

**如实说明现状**：这条协作纪律依赖的落地即验工具是 **T44**（`docs/BRAIN_TASKBOARD.md`
所列，属主 v4，撰写本文件时状态为 **DOING，尚未交付**）。T44 要做的是"per-run 实时
验证脚本：run 落库→契约门+radio 覆盖+出口读出→单行判词，外场自检零判断成本"——
这正是本文件①"自检"一节里那几条命令的自动化/实时化版本。**T44 交付前，①的自检
命令需要操作者自己手动跑**；T44 交付后，本节描述的协作节奏才算真正生效，届时本节
需要更新为 T44 的具体调用方式（不在本次交付范围内，登记为跟进项）。

**T44 就绪后的协作节奏**（目标态，供 PO 提前了解怎么配合）：
1. PO 跑完一条 run（`RUN_END ... status=completed`）；
2. T44 的实时验证在服务端/大脑一侧自动对这条 run 跑契约门+radio 覆盖+出口读出，
   给出单行判词（类似"PASS"或指出具体哪项不过）；
3. 大脑通过既有的跨会话消息渠道把判词转达 PO（具体转达方式——是否需要 PO 主动
   查看某个文件/等待通知——由 T44 交付时一并定义，本文件不预先猜测）；
4. 若判词不过，PO **当场重采**（新 run_id），不必等回到办公室才发现这轮数据有问题
   要重新跑一趟；
5. 若判词过，PO 直接进入下一格/下一轮，不需要额外确认。

**操作者需要做的唯一准备**：确保①"到点"步骤里的实时 logcat 落盘与哨兵机制正常
工作——T44 的"实时"依赖的是 run 落库后能被尽快读到，操作者这边不需要额外操作，
只要不跳过①②的既有步骤即可。
