...
## 7. 流量消耗确认开关（大脑裁定 spec §8.7-7，本节为新增，v2 lane 负责实现）

**裁定**：`s4_throughput` 默认 **WiFi-only**，蜂窝网络下需要**显式确认开关**才能执行——512MB 下行
ceiling 是真实的用户流量成本，这是产品责任而非技术判据，spec 本身不定义 UI 交互细节，本节只交代
v2 需要接的行为契约。

**行为契约**：
- 客户端在**蜂窝网络**下准备执行 `token_experience` run 且该 run 会触发 `s4_throughput` 场景时，
  须先向用户显式确认（例如一次性弹窗或设置项"允许蜂窝网络下测吞吐"），用户未确认时**跳过**
  `s4_throughput`（其余 s1/s2/s3 场景不受影响，正常执行）——即整个 run 依然产出，只是
  `scenarios[]` 里不会有 `profile_id=="s4_throughput"` 的条目（与 §3"出现条件"里"缺 profile 静默
  跳过"是同一种可观测结果，v2 侧无需区分"因为没装 profile"还是"因为蜂窝未授权"两种跳过原因）。
- **WiFi 下不需要任何确认**，直接按现有诊断流程正常执行。
- 具体交互形式（一次性弹窗 vs 设置页开关 vs 每次询问）由 v2/产品侧决定，本文不做限定；spec 侧
  （批③ App 实现）只需要提供一个可供 UI 层判断"当前是否在蜂窝网络"的既有能力（`network_snapshot`
  已有 `transport` 字段可读）与一个"是否执行 s4_throughput"的入口开关，具体挂哪个 Activity/Fragment
  由 v2 决定。
- 这个开关是**纯客户端侧决策**，不影响本文档 §1-§6 定义的任何 wire 字段——服务端/schema 不需要知道
  "这次跳过是因为流量确认还是因为版本没装该 profile"。
