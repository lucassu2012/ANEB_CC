...
> 🔴 **本文件只是预案，写完不执行；不动设备直到窗令。**
> **执行前置＝下列三条，三者未齐不得起步 1**（2026-09-06 现态：**三条全未落地**）：
>
> | # | 前置 | 落地判据（不是「说过了」） |
> |---|---|---|
> | ① | **窗令登记**：T80 记入新 DW 号 | **有机器判据**：采集器真机门要求 `--device-window` 在 `docs/BRAIN_TASKBOARD.md` 上查得到，未登记即 `REFUSE`、**退出码 3、一个文件都不建**（2026-09-01 本窗实撞） |
> | ② | **PO 跑** `docs/E2_SCHTASKS_SETUP_20260906.md` §2（schtasks 常态化，PO 已裁 (b)） | 该 §2 的验收项逐条绿 |
> | ③ | **v4 段 B 自环 README 落仓** | `evidence/b2_selfloop_<ts>/` **在库可查**（零设备，PC 内 clumsy＋WinDivert 自证） |
>
> ⚠ **①的机器判据只拦「窗 ID 没登记」，拦不住「登记了但不该现在跑」**——
> 后两条**没有机器判据，只能人核**。**别因为采集器起得来就当三条都齐了。**
