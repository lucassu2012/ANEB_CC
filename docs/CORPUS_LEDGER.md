...
- 带 AQS **分数**的 run（`run.aqs.score` 非空）：99；其中 low_confidence：99/99（100%）｜顶层 `aqs_version` 版本戳共 101 条，其中 **2 条只有版本戳、没有分数**（两个量不可混用）
  ...
  > ⚠ **本台账无法从语料判定低置信的成因**：wire 记录是平铺的（`t1_ttft_ms`／`t2_itl_p95_ms`…），**不带逐 KPI 的 `sample_count` 与`low_confidence`**，run 级只留一个布尔 ⇒ 拿着这 99 个真实 run 也查不出是哪个 KPI 把它钉成低置信的，须查 `AqsInputMapper`。（写出来是为了省下下一个人徒劳查一遍的时间。）
