...
| T9 | **CGNAT 出口 IP 契约化**（承 GUARD_DIFF D-1 裁定 + D-374「制式与出口共线」，v2 的 23:00 前间隙任务，T2 优先） | v2 | DOING | ① 先盘点：server 现有端点/响应是否已回带 client observed addr，能复用则零 E-01 改动；**若需 E-01 变更→停，报大脑+PO 走受保护流程**；② 契约加性可选字段（network_snapshot 下，命名自定+来源标注），旧语料照常通过=真加性；③ App 采集落盘；④ 读者同日出生（D-276）：radio/validity 面呈现出口 IP+窗口对照，CSV 列；⑤ 守卫+突变审计+D 号入册；⑥ 缺失=合法状态，分析层不得因缺失改分母（D-336） | 待填 |
