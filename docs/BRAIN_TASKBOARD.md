...
| T11 | **6-4 `validated_against_version` 落地·spec 侧**（承 T7，裁定 6-4） | v4 | DOING | ① 两个 adapter JSON 加 additive 字段（validated_against_version/versionCode+采集命令注明 `dumpsys package` 只读）；② README 补该字段的生命周期与 STALE 降级语义；③ check 侧守卫（字段存在性+格式）；④ **App 宿主判断逻辑（比对+STALE 标注）列为交接项给 v2，本任务不动 app/**——文件面与 v2 的 :probe 零重叠 | spec/adapters/ 待填 |
