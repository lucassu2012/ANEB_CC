...
| T8 | **GUARD_DIFF P0 修复包**（承 T6，裁定见「大脑裁定」第二批） | v3 | DOING | ① C-1：round_effect 汇池前提守卫（D-335 同款）+突变审计，C-3 随修验证；② B-1/B-3/B-5：`SCENARIO_INTRINSIC_JITTER` 三面落地+摘要拆分+常量归档；③ B-2：--plan 结论句拆 s2；④ A-1(a)：plan 表接进报告三面；⑤ A-2 落定=取证 5 轮（改增补草案 §8 #3）；⑥ test_cli_smoke Py3.14/Win 失败处置；纪律=每条立 D 号→改→突变审计→三面核对渲染，验收=大脑复核+套件全绿或如实说明 | 待填 |
