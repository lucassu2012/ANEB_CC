...
| T10 | **步骤 D：Profile 4 语音回环规格先行**（执行序 D 正式启动；v2 第二间隙任务，23:00 T2 优先） | v2 | TODO | ① 盘点先行（T9 范本）：server `/v1/voice/echo` 端点现状、App 音频采集/播放能力现状、profile 4 YAML 是否有骨架——先数清已有的再写缺的；② KPI 定义与口径边界：应用层语音回环时延等，声明边界照 claim_boundary 纪律（不称无线口/运营商指标）；③ 打点锚点借 INSTRUMENTATION_SPEC 三段式定义法（语义真值≠可观测判据≠E_anchor）；④ 误差预算骨架+验证实验设计（设计即可不执行）；⑤ 不碰设备、不改 E-01——端点若缺失，写清所需 server 增量列为待裁定（E-01 变更走受保护流程）；⑥ 23:00 未完即标 DOING 留明日，T2 绝对优先 | spec/ 或 docs/ 待填 |
