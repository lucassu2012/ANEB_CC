...
| T10 | **步骤 D：Profile 4 语音回环规格先行**（执行序 D 步启动；T2 绝对优先，未完标 DOING 留明日） | v2 | DOING | ① 盘点先行（server `/v1/voice/echo` 现状、App 音频能力、spec profile 4 骨架）；② KPI 族+口径边界（只称应用端到端，不碰 VoLTE/MOS）；③ 打点锚点三段式（语义真值≠可观测判据，差=E_anchor）；④ 误差预算骨架+验证实验设计（录放缓冲时基预计为主项）；⑤ 不碰设备、不改 E-01，缺口列为待裁定 | 待填 |
