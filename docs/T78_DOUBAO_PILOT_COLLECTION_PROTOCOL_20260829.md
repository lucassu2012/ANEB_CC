...
### (2)-A 受控变量：每轮必须一样的那三样（2026-08-29 补——「被执行面」清单数出的三条缺口）
...
```
wifi_f1/ cell_f1/ …       每格一目录（脚本 --out；命名取值见 §2-A G-1：
...
| 某格的 5 轮提示词**不逐字相同**，或中途**沿用了同一个对话** | 违 §2-A G-2／G-3——工作负载或上下文长度成了未受控自变量 | **该格作废重跑**。⚠ **锚格若犯，整批漂移判据作废**（它的全部意义是与块 1 同负载） |
...
4. 产物落 `evidence/doubao_wave0_<日期>/` 并写 README。
   ...
   > | 3 | **每次网络切换的时间戳** | §2-A G-4 | 同上；且块中途切换只能靠它发现 |
   > | 4 | **每功能的提示词逐字** | §2-A G-2 | 工作负载不可复算；锚格是否真同负载无从判 |
   > | 5 | **F3/F4 素材的文件名与字节大小** | §2-A G-2 注 | 上行结论不可复算，且事后补不回来 |
