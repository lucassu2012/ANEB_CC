...
**收窗五步**：停本次起的一切（含 PCAPdroid／a11y 观察）→ 只恢复**本次改过**的（`deviceidle`／`stayon` **本批都不动**）→ 回华为桌面复验干净 → 产物落 `evidence/doubao_wave0_<YYYYMMDD>/` 并按 **README 必填 12 项**写 README → 板面更新＋窗后待办（§5）。
