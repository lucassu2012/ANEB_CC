...
| 蜂窝两块**跑完** | 该块内是否**全程同一制式** | README 的「全部偏离」段 |

- **全程同一制式** ⇒ 蜂窝条件成立，照常判读；
- **中途漂过** ⇒ **不作废**，但该块的条件对照**必须按「混合制式」标注并降级**，
...
4. 产物落 `evidence/doubao_wave0_<日期>/` 并写 README。
   ...
   > | 3b | **每格开跑前的 `gsm.network.type` 原样值**，以及蜂窝两块是否全程同制式 | §②-A G-5 | **NR↔LTE 漂移不记就永远看不见**，而它会让「蜂窝」变成两个条件——D-393 的原样重犯 |
