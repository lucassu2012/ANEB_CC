...
7. **交接给判读侧（本条 2026-08-29 补——此前 ⑤ 只有「板面更新＋简报」，那是广播不是定向交接；`v3` 在本文出现十余次却全是上下文引用，没有一条说「采完之后谁通知谁、带哪些限定」）**：
   ...
   > | ③ 判读口径入口 | 本文 **§3** | 含「本批默认只用 `e2_analyze`、`e3`/`e4` 待一格实语料验」与**核心量在分析侧零读者**那条 |
