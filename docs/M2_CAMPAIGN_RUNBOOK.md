...
**③ 每格正式采集前，先跑一轮「丢弃的预热轮」（PO 批复 D-366，试点报告 §2.4 选项①）。**
预热主因是**无线唤醒**（蜂窝 2.4~3.6× 于 WiFi，D-358）：一轮丢弃的 quick run 把它移出
第一条计入的 run。操作与记账：

- 预热轮照常启动（同命令），**RUN_END 后把它的 `run_id` 记入台账并标「预热丢弃」**；
- 它**会**上报进服务端——**拉取时按台账排除**，绝不进语料（这就是台账为什么必须记它）；
- 预热轮之后**紧接着**跑计入的 n 轮（间隔别超过 ~1 分钟，射频回落预热就白跑了）；
- 此后计入 run 的绝对值口径 = **「射频已热、App 冷」**（App 每轮仍重启，
  ~4% 的 App/TLS 残余仍在，D-358）——报告正文标口径时照此写，别再写「冷启动口径」。
