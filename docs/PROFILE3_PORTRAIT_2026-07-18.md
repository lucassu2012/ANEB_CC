...
- **本层 = UI 呈现口径**：无障碍事件观察到的「**A0′ 用户气泡上屏** → 响应首字上屏」延迟，**含 App 渲染管线**，
...
- **起点订正（D-396 / 大脑裁定 6-6 前半，2026-08-01）**：本节原写「发送 → 响应首字上屏」，
  ...
  §1 表里的 TTFT 一列不得被读作「用户按下发送到看见首字」**。
  本次只订正措辞，**一个数值都没改**；更大的口径改动（改字段、拆列）待 E3 出数（裁定 6-6 取②）。
  连带范围——**这里不能一概而论**（本句原写「三者起点同为 A0′」，与 `INSTRUMENTATION_SPEC` §1.4
  直接相反，且替一个尚未裁定的问题下了结论；T14 交叉审查揪出，此为订正）：
  **豆包/千问**走 v3 簇分割，起点确为 A0′；**DeepSeek 不是**——它的 `ttft_method` 是
  `v3-cluster + v4-density`（`deepseek.yaml`），**长回复**时 v3 才闭合（那一支起点是 A0′），
  **短问答**时 v3=null、只有 v4 密度谱出值，而那个起点是**密度谱切换点、不是 A0′**
  （D-51/D-53/D-55；§1.4 逐字："A0′ 不可得"）。故 §1 表里 DeepSeek 的 500 ms（出自 D-55 短问答）
  与豆包的 1984 ms **起点语义不同，不可并读为同一口径**。
  本条对 §1 整张表**确实**生效的只有一句：**每一格都不得被读作「用户按下发送到看见首字」**；
  至于各格起点究竟是 A0′ 还是密度谱切换点，属**裁定 6-6 的第二半，尚未裁定**，E3 出数前不下结论。
