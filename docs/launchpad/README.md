...
- observationId 碰撞风险（spine-1）：TokenObservationExport.observationId = `apiprobe-<provider>-<ms>` 不含 subject，同毫秒多 subject 采集会产生同 id → Codex `_validate_partition_disjointness` 报 duplicate_observation_id_within_partition 拒整分区。修：加 subject 短哈希/单调序号。
