# 补丁：判据 V4 拦窗 #9（§2-9／§8-5）零读数判别量去常量化（v2）

> 🔴 **v2 订正（2026-09-26）**：v1 有一条门 `test_dead_reader_a1_refuses_everything_downstream` 断言「全部不可判」——**那把 V4 §8-19 的缺陷钉成了期望**（`apply_verdicts` 在 A1 缺席时整段早退，把有自己前提的 FORWARD 层 C1／H2／N／底噪一并丢掉）。谁去修 §8-19 都会先撞上这条门变红。由核状态工作流的汇总者与一名怀疑者**独立**指出。
> v2 改为 `test_dead_reader_a1_is_refused_and_no_a1_identity_verdict_is_printed`：只断言 A1 本格被拒、不印 A1 身份判词。**已实证不再钉住 §8-19**：模拟一次 §8-19 修法（A1 缺席不整段早退）后本门仍绿，输出中 C1 按自己的前提照判身份。M3 仍被抓（由 S1 那条门）。
> ⚠ 验证时我第一次写的「模拟修法」本身有缩进错，门因此红——先查了仪器才下结论，正确模拟下门绿。
...
- `apply_verdicts`：死掉的 S1 必须先印 NOT_EXECUTED 且不再印底噪判词；死掉的 A1 被拒判且**不印 A1 身份判词**（刻意不断言「下游全部不可判」，见下方 v2 订正）；句柄 b 死 ⇒ VOID_MISSING；句柄 b 真零 ⇒ 仍判 ZERO（反方向）
