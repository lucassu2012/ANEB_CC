...
## 9. 勘误与限定（2026-09-06，承上游 D-718..D-726）

1. §3.7 F7-03/F7-07 与 §7.1 A-5 中「DW-02 wifi_f6（p99 17,230 ms）」指 wave0 子批 **DW-20260830-02** 的 wifi_f6，不是 DW-20260905-02；数值存在（D-726）。
2. §3.1「L1-missed：U1 字节对账在 serverView 解析失败时 fail-open」按 D-721 限定：U1 是完整请求，2xx 即整体已收、时长锚在响应头，不依赖 serverView；只有按窗截断的 U3 必须用服务端视角。建议改为「serverView 缺失只置 U3 为 null 并记诊断字段」——上游 50df3e7/0fb4c77 已按此落地。
3. 语料口径两个量不可混用（D-722/723）：**真实 run 总数** 113→101 与 **带 AQS 分数的 run** 111→99；本报告 §1「111→98/99」与 §7.1 A-4 验收「台账 99」指的是后者。A-4 已按此落地并核收。
4. `-race`：协调侧在 Linux 鲜克隆对上游 `3235779`（含 A-8② TLSNextProto 改动）实跑 `go vet/build/test -race` 全绿，补 D-719③「本机无 gcc 未跑」。
5. 执行对账（09-06 12:37 +0800 时点）：A-0/A-1/A-4/A-5/A-6 DONE；A-8①②③ 代码完成、真机验证挂 C-2、C-3 升为收口前提（D-725）；三句裁定 + PO 四裁全部入册（D-706/707/708/712）；C-1 v0.1 自然对照版成稿候复核（D-720/724）；未动：A-2、A-3、A-7 段 B/C（E-2 创建单待 PO）、B-2。
