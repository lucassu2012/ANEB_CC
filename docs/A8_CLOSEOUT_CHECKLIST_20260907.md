...
| 7 | **`AnebClient` 逐样本记 `response.protocol` 与 `X-Aneb-Proto` 上报（additive）** | 🔴 **未交付**：`X-Aneb-Proto` 在 `app/` **零命中**；`negotiated_protocol` 在 `app/ scripts/ server/ spec/` **零命中**；`AnebClient.kt` 无任何 `protocol` 读取。服务端侧已在发（`server/h3.go` 的 `withProtoEvidence` 里 `w.Header().Set("X-Aneb-Proto"`），且该文件头注释自述「**客户端一侧逐样本记录**」＝对侧存在而客户端半件没写 | **L0** | 八笔 A-8 提交无一涉及 |
...
| **V4b** | `negotiated_protocol` 全为 `http/1.1` | 🔴 **不是真机阻塞，是发射端不存在**（见第 7 件）。**D-719③ 把 V4a 与 V4b 并列挂 C-2，那条挂错了层**——即便明天开真机窗，这一条也不会有值 |
...
1. **A-8-OPEN-1**：客户端逐样本记协商协议（`response.protocol` ＋ `X-Aneb-Proto`）**未交付**；连带 V4b 不可达。**这不是真机项。**
