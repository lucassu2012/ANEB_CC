...
> ⚠ **本节是方法学参考，当前不生效**：部署形态已于 **D-48** 收为**单实例 E-01**（放弃三级
> 同城/区域/中心差分），2026-07-29 PO 再次确认「利旧现有服务器、不再增加新服务器」。
> 下面这套差分在代码里仍实现着，恢复多层级部署即可用；**在此之前，报告的归因段只有接入段
> 绝对值**，并在段顶印明「三级差分的骨干分解本轮不可得」。外场作业照
> [`M2_CAMPAIGN_RUNBOOK.md`](M2_CAMPAIGN_RUNBOOK.md) §2 走——**别照本节去排三层级行程**（D-311）。
