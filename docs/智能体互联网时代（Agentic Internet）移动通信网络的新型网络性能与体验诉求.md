...
本部分将前文分析收敛为一套可操作的 KPI 体系，作为测量工具（见《ANEB Probe 开发设计文档》）的需求输入。体系版本号 **agent-qoe-kpi v0.2**（v0.1 为 2026-07-11 定稿基线；v0.2 于 2026-07-12 按《测量红队清单》32 项经对抗验证的测量失真风险修订统计口径与有效性规则），门限值标注为"实验性"——待阶段一实测数据回流后修订。
